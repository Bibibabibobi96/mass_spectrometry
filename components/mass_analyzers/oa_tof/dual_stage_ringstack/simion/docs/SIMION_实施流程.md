...
加速器轴放在`x=-48.8 mm`，反射镜轴放在`x=0`；接地实体detector的中心位于
`x=+48.8 mm`，半径40 mm、厚1 mm，迎离子上表面位于`z=L_accel`。禁止用Lua测试面
代替检测器实体；Lua只允许在第4实例真实电极splat后的`segment.terminate`中记录命中。

同名正式Fly2必须在GUI中显示100 amu、+1、N=1000、1×1×1 mm³释放体和`5±1 eV`
