...
当前`oatof_accelerator_3d.gem`的9号接地屏蔽是完整外方框减内方框并另加后封闭，没有沿局部横向
注入轴切除侧孔。RF连接候选不能只在IOB中把粒子放进屏蔽内部；必须由同一候选参数生成与COMSOL一致
的侧孔和接地注入管，再重新Refine、Fast Adjust并验证孔边缘场、粒子捕获和GUI可见实例。
