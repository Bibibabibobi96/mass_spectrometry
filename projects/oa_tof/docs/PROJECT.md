...
1. **完成候选运行编排与逐变量运行时覆盖。** `config/design_variables.json`、可审查扩大的
   ...
   `zero_change_structural_candidate` execution profile现已把已验证runner绑定到solver-neutral计划：只有
   获批524 Da、N=100、零变量变化、仅传输目标且显式提供同request/同run_id候选计划时才返回
   `EXECUTION_READY`；缺少绑定返回`NEEDS_RUNTIME_INPUTS`，分辨率、500 Da或任何变量仍返回
   `NEEDS_IMPLEMENTATION`。下一步按重建影响选择代表性非零变量做小规模运行时覆盖，每项通过后再扩展
