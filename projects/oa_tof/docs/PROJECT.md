...
| 平均TOF (us) | 71.98704572 | 71.99022582 |
| 直接质量FWHM (Da) | 0.01359256443 | 0.01874021531 |
| 质量分辨率R | 38550.49 | 27961.26 |

标准化KDE重叠为`0.8002`，bootstrap绝对R差异的2.5%分位为`15.604%`。两端命中率和平均TOF高度
接近，但峰形及直接FWHM存在稳定差异；不能为追平单一R而调网格、时间步、quality或场参数。
...
严格聚焦几何提升另有同源N=1000资产比较：两端均1000/1000命中，平均TOF差`0.84220 ns`、逐粒子
TOF RMS差`1.12383 ns`、落点RMS差`0.14466 mm`。这组结果用于证明提升资产的数值行为，但其完整
身份尚未写入`formal_validation.json`，因此不得与上表的机器冻结性能无标注混用。
...
1. **先消除正式验证身份歧义。** 核对严格聚焦N=1000提升结果与当前正式MPH、SIMION包和粒子表的
   SHA；若它们确为当前正式资产，生成新版`formal_validation.json`并通过Formal门禁。完成前不再
   在README等入口重复任何性能数字。
