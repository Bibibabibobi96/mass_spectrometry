...
1. **LiveLink启动瞬态分类。** 只对可证明发生在任务逻辑前的服务器未连接做有限干净重试并保留
   每次失败；Study Compute、MATLAB任务和原生崩溃不得被自动重试掩盖。
2. **COMSOL极小粒子数原生崩溃深挖。** N=40正式候选已绕开阻塞，但build 293下N=3仍稳定在
   `Xmesh.assemInit`崩溃。后续按项目COMSOL文档中的已冻结诊断矩阵继续查找最小失效粒子数和
   solution-mesh初始化条件；不得把当前可用绕行写成内部根因已完全证明。
3. **加速区纵向差异复核。** 先复用现有场样本区分场插值、边界几何和z-to-TOF映射，只有现有证据
   无法判别时才增加一个定向COMSOL网格对照。
4. **SIMION透明栅网去网格相位化。** 若继续追求亚ns逐粒子闭合，实现自动越过实际数值电极层，
   并证明对网格相位收敛；不能继续扫描固定跳转距离追平单一R。
5. **按需发布复现ZIP。** 从正式自包含目录生成不含日志、收敛参考和临时轨迹的ZIP及独立SHA；发送后
   ZIP可删除，源码构建链和正式目录继续保留。

真实丝网局部单元、制造与装配误差预算、RF接口及二维轴对称混合模型均按当前决定暂缓，不进入本轮
程序/分析故障清理。
