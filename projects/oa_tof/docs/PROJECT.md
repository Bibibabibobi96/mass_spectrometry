...
1. **建立候选参数编译层。** 后期优化允许把反射器各级电极片数量、厚度、内径/孔径、外径、间隙、
   屏蔽罩内径/厚度及电压等声明为变量。连续尺寸、整数数量和拓扑变更必须分类；总长度和位置等派生量
   不能与其来源同时独立调整。每个变量还需声明范围、步长、耦合约束和是否触发CAD重建，并经候选
   `baseline → resolved → COMSOL/SIMION/CAD`单向发布后才可执行。当前
   `config/execution_profiles.json`只映射固定五质量和现有524 Da正式复验，因此含上述变量的dry-run
   应报告`NEEDS_IMPLEMENTATION`，不能拿现有正式模型冒充优化结果。
