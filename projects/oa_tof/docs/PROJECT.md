...
1. **把静态候选合同接入求解器/CAD候选入口。** `config/design_variables.json`、可审查扩大的
   ...
   也能在求解前拒绝缩短后未同步重配而发生的电极重叠；测试数值不构成候选设计建议。当前
   `config/execution_profiles.json`仍只运行固定五质量和524 Da正式复验，因此下一步是让候选入口显式
   消费编译产物，再闭合COMSOL/SIMION/CAD；在此之前dry-run继续报告`NEEDS_IMPLEMENTATION`。
