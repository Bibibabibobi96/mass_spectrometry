...
1. **完成候选运行编排与逐变量运行时覆盖。** `config/design_variables.json`、可审查扩大的
   ...
   尚未启动商业软件做零改动端到端冒烟，因此当前接受范围仅为`structural_build_and_contract`且禁止性能
   声明。下一步是完成一次零改动真实候选运行，再把已验证runner绑定到通用execution profile；此前
   通用dry-run继续报告`NEEDS_IMPLEMENTATION`。
