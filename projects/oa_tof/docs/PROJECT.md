...
1. **完成候选运行编排与逐变量运行时覆盖。** `config/design_variables.json`、可审查扩大的
   ...
   尚未物化，因此当前Candidate仍会在模板前置门禁失败，且不会读取旧Formal资产。Git内声明式W-GEM生成器
   从唯一`resolved_geometry.json`派生四槽顺序、transform和优先级；用户必须用新SIMION GUI把该W-GEM
   物化为独立同basename IOB/CON，再以`simion/workbench/register_candidate_layout_template.ps1`登记。
   Candidate只接受W-GEM、三件套、结构报告和来源SHA均有效的登记run，不接受旧Formal、archive/history、
   旧Candidate或任意裸IOB/CON路径。
