...
统计闭合另以同源N=1000只读加载该MPH并复用`sol1`静电场，只重算`std2`。N=1000粒子阶段
`714.95 s`、1000/1000命中、平均TOF `71.9870457216 us`，统一直接KDE FWHM为
`0.0135925644303 Da`、`R=38550.49`。逐粒子结果和与SIMION的正式比较由
...
候选MPH位于artifacts的COMSOL candidates目录。复用其独立`sol1`并把平移后的同源N=100粒子
写入GUI可见`ReleaseFromDataFile`后，`std2`在`382.73 s`内完成，100/100命中，初始位置最大误差
`7.1e-15 mm`，平均TOF`71.9901770195 us`，统一直接质量R为`31611.76`。同点场比较未显示相对
正式几何的闭合改善，因此没有提升为正式MPH，也没有触发CAD同步。
