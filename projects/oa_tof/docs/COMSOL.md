...
日常冒烟使用已验证N=40；N=30只作为当前模型、固定序列与build 293的最低实测成功点，不是已
...
能量在CSV记录精度下也全部零差。因此逻辑上只需1--29个粒子的调试，可以求解已验证的N=30承载
集合（日常仍优先N=40），再只分析目标前缀ID；运行记录必须同时声明`solver_particle_count`和
