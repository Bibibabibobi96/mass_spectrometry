...
修改。诊断按最小改动原则开展：先用单区域或单分量筛查，再仅在证据要求时组合多个选择；每次运行
记录规范化掩码、真实场基线、唯一改动及TOF/落点/透过率响应。组合掩码可以证明联合响应，但若没有
配套单因素对照，不能把结果唯一归因于其中某一分量。该方法目前只在oa-TOF项目实现，尚未经过第二
项目验证，不提升到仓库通用验证方法。
...
| 500 Da、N=25/27/28 | 同一正式路径、固定种子、独立进程 | 均在`Xmesh.assemInit`原生FAIL；N=25约140.8 s，N=27/28约93 s |
| 500 Da、N=29首次 | 同一正式路径、固定种子、独立进程 | Study返回解尺寸，首次`mphparticle(t=0)`时JVM GC线程原生FAIL |
| 500 Da、N=29复跑 | 与首次完全相同 | 设置分段输出表达式时JVM GC线程原生FAIL，未进入Study |
| 500 Da、N=30 | 同一正式路径、固定种子、独立进程 | PASS，30/30，粒子295.58 s |

质量、时间窗、常规OOM、线程数、分配器、`clearSolutionData`和`pp1`重写均已排除为必要条件。
当前固定序列中N=25/27/28均在solution-mesh初始化失败，N=29两次均失败但阶段不同，N=30及
N=40以上已测样本全链路成功。N=29首次证明Study本身可以返回，但复跑又在Study前失败，因此不能
把首个Study成功或首个全链路成功解释为严格、确定且只由N控制的内部阈值。工程绕行收紧为
日常检查统一使用N=100；N=30只作为当前模型、固定序列与build 293的最低实测成功点，不是已
证明对所有粒子表或N=31--39单调成立的通用下界。
详细旧时间线已冻结到项目history；本阶段新增证据保留在各自独立artifacts运行目录。

首次N=30阈值尝试在任务报告创建前被会话中断；随后若干尝试停在MATLAB/LiveLink启动阶段，均未
进入项目脚本或Study Compute。`retry4`保存的launcher stderr最终显示用户`.comsol`配置/Tomcat
日志拒绝写入；同一入口在允许写该目录的执行环境中，`retry5`完成30/30并通过manifest。因此这些
启动失败的根因是编排沙箱权限，不是N=30、用户COMSOL配置损坏或求解器结果。共享启动器仍以首份
任务报告为120秒启动门禁，超时只终止本次进程树、清理本次新增服务器并有限重试。
...
N=3与N=40两个端点之间未经测试的区间假设为单调。报告现分别记录`study_started`、
`study_completed`和`initial_release_read_completed`，将启动、任务配置、Study、结果提取和后处理
失败分开；旧报告中的`PARTICLE_SOLUTION_DATA_CLEARED`与`SOLUTION_SIZES`仍可向后兼容判定阶段。

当前模型还具备一个不改运动方程的低N诊断绕行。CPT只包含`ReleaseFromDataFile`、外加静电场的
`ElectricForce`和探测器`Wall/Freeze`，没有空间电荷、库仑相互作用或其他粒子间耦合。同一种子下
N=3表与N=30表的前三行完全相同；成功的N=30与N=40解中，前三个粒子的TOF、落点、初始条件和
能量在CSV记录精度下也全部零差。因此逻辑上只需1--29个粒子的调试，可以求解统一N=100承载
集合，再只分析目标前缀ID；运行记录必须同时声明`solver_particle_count`和
`logical_particle_count`。若今后启用空间电荷、粒子间碰撞或任何集体效应，该绕行立即失效，必须
重新验证。它解决低N工作流阻塞，但不代表build 293的原生缺陷已修复。
