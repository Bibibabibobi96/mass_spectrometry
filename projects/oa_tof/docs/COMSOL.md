...
0.5 mm固定N=100候选的静电求解约19.1秒、粒子求解约1478.6秒，100/100命中；直接FWHM得到
`R=28837.88`，只比正式结果低1.43%。焦点顶点只从`1.58797`移到`1.58895 mm`，COMSOL仍有2个
显著KDE模态。当前正式粗网格的检测器RMS落点半径为`4.4226 mm`、相对SIMION质心偏移
`2.4579 mm`；0.5 mm候选为`3.5644 mm`，与SIMION的`3.5736 mm`接近，质心距离降到`0.1041 mm`。
结论是横向伪场已定位并收敛，它显著影响落点但不是峰肩主因。候选未保存为MPH、未覆盖正式模型；
本轮没有几何变化，故无需更新SolidWorks装配体。
