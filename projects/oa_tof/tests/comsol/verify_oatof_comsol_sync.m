...
assert(strcmp(char(shieldOuter.getString('h')), ...
    'L_flight+L_refl+ring_thickness+shield_axial_gap-(z_accel_origin-accel_repeller_thickness-accel_shield_back_extra-accel_shield_wall-endcap_gap)+2*flight_tube_wall'), ...
...
meshFeatureTagArray = string(cell(mesh.feature.tags()));
meshFeatureTags = join(meshFeatureTagArray, ',');
assert(~contains(meshFeatureTags, 'szbracket'), ...
    'Obsolete whole-accelerator submillimetre mesh feature still exists.');
szaccelIndex = find(meshFeatureTagArray == "szaccel", 1);
ftetIndex = find(meshFeatureTagArray == "ftet1", 1);
assert(~isempty(szaccelIndex) && ~isempty(ftetIndex) && szaccelIndex < ftetIndex, ...
    'Formal accelerator Size is absent or ordered after Free Tetrahedral.');
assert(abs(model.param.evaluate('mesh_hmax_accel')-1e-3) <= 1e-12, ...
    'Formal accelerator mesh hmax is not 1 mm.');
assert(strcmp(char(mesh.feature('szaccel').getString('hmax')), ...
    'mesh_hmax_accel'), 'Formal accelerator Size is not parameter-linked.');
fprintf(fid, 'MESH_FEATURE_TAGS=%s\n', char(meshFeatureTags));
meshInfo = mphmeshstats(model, 'mesh1');
assert(meshInfo.numelem(2) > 300000, ...
    ['Formal 1 mm accelerator mesh has only %d tetrahedra; the Size ' ...
     'feature is present but did not control the built mesh.'], ...
    meshInfo.numelem(2));
...
assert(abs(model.param.evaluate('cpt_dt_fine')- ...
    ...
    'Formal fine output step differs from baseline.json.');
assert(abs(model.param.evaluate('cpt_dt_drift')- ...
    runtime.field_free_output_step_ns*1e-9) <= 1e-15, ...
    'Formal field-free output step differs from baseline.json.');
...
for k = 1:numel(requiredTimeTokens)
    assert(contains(tlist, requiredTimeTokens{k}), ...
        'Formal Study Output times omit %s.', requiredTimeTokens{k});
end
solverTime = model.sol('sol2').feature('t1');
assert(strcmp(char(solverTime.getString('tstepsbdf')), 'free') && ...
    strcmp(char(solverTime.getString('tout')), 'tlist'), ...
    'Formal GUI solver storage/step settings are not the validated policy.');
...
assert(~cpt.prop('StoreExtra').getBoolean('StoreExtra') && ...
    ...
    'Formal CPT storage checkboxes differ from baseline.json.');
