...
分段加速模式的相位匹配 RF 幅值—频率筛选及 N=1000 复核显示微小排序不稳定，维持 P0 RF 基线，
不据此优选参数或扩大扫描；依据见 [RF 复核报告](history/20260803__hex-rf-drive-phase-matched-h15-n1000.md)。
不同初始源能量与加速方式的描述性比较见
[四模式源能量记录](history/20260803__multipole-four-mode-source-energy-h15-n100.md)；数值只在该报告维护。
...
连接 oaTOF 时，campaign 须显式选择公共接地圆套筒与带孔法兰 profile；不开放屏蔽电位作为隐式变量。

1. 按[通用出口相空间方法](../../../docs/multipoles/exit_phase_space_control.md)建立具名筛选campaign；
   当前P0 RF基线在新合同批准前不变。关闭条件是冻结真实下游接受尺度、唯一变量、损失约束、共同
   幸存者分析和相邻数值档。
2. 使用当前注册的 runtime profile和声明式campaign入口完成真正需要的N=100/N=1000实验，不恢复一次性
