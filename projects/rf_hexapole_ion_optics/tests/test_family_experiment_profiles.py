...
class FamilyExperimentProfileTests(unittest.TestCase):
    ...
    def test_preregistration_refuses_unsourced_numerical_thresholds(self) -> None:
        ...
        self.assertEqual(
            ...
            "exit_interface_nosweep_mesh_strategy_field_pilot_must_close_before_particles",
        )
    ...
    def test_d1_mesh_build_is_bounded_and_d2_is_only_conditional(self) -> None:
        ...
        self.assertIn("sweepRodBoundarySelectionTags{index}", mesh_helper)
        self.assertIn("'sel_mesh_rod_bnd'", mesh_helper)
        self.assertIn("strategy.radial_core_and_rod_hmax_mm", mesh_helper)
        self.assertGreaterEqual(mesh_helper.count("if ~localized"), 2)
        self.assertNotIn("'szTetInterface'", mesh_helper)
