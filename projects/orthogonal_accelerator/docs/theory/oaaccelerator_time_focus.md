...
按问题定位：[坐标与电压](#3-坐标电压和几何约定) · [能量与单位](#4-能量每电荷与单位) · [分段时间](#6-分段飞行时间) · [一阶焦面](#7-一阶空间时间聚焦面) · [紧凑边界](#8-d_a--0-的特殊紧凑边界) · [程序接口](#9-参考程序接口) · [参考测试](#11-最低参考测试)。

## 1. 文档职责

离子带非零初速度且存在实测线性 $z-v_z$ 相关时，使用
[`affine_phase_space_time_focus.md`](affine_phase_space_time_focus.md)；本文以下静止释放推导不重复扩写该模型。

本文定义独立正交加速器项目中双区结构的一维、分段均匀场参考模型，给出：
...
本文不保存当前项目电压、尺寸、候选状态或正式结果。当前事实仍以项目 `config/` 和 [`docs/PROJECT.md`](../PROJECT.md) 为准。本文从单次反射 oa-TOF 项目迁入；此前取代的[`三栅加速器总长度符号推导.docx`](../../../single_reflection_oa_tof_mass_analyzer/docs/history/20260721__superseded-theory-docx/三栅加速器总长度符号推导.docx)仍按原项目身份只读保留，不能继续作为当前公式权威。
...
上述名称用于理论叙述；机器 provider、API 与结构变体标识只由
[component_contract.json](../../config/component_contract.json) 定义，不从理论名称派生第二套 ID。
...
本节含速度与实际时间的公式采用 SI：长度用 m、场强用 V/m、质量用 kg、电荷用 C、时间用 s。
使用下文以 mm 命名的程序接口时由实现完成单位换算，不能直接把 mm 代入 SI 公式。
...
- $z_{A,\mathrm{out}}$：双区正交加速器末级场出口参考面；
- $z_{A,f}$：一阶时间聚焦面；
- $D_A$：两者之间的有符号距离。
...
改变 $V_R$、$V_G$、$g_1$ 或 $g_2$ 后，必须重新计算 $D_A$ 和 $z_{A,f}$。不得把聚焦面永久硬编码为出口面。

装配到仪器时，位置由完整刚体变换、速度由其中的旋转映射。加速器项目不规定整机的全局原点或
提取方向；例如 MR-TOF 的局部正提取方向可映射到项目 $-z$，中央 $z=0$ 焦面要求由 MR-TOF 合同提供。
...
单次反射 oa-TOF 的整机优化必须使用其
[`加速器—反射器联合时间模型`](../../../single_reflection_oa_tof_mass_analyzer/docs/theory/oatof_oaaccelerator_coupling.md)。
MR-TOF 的镜、Stripe、棱镜与多圈返回条件由 MR-TOF 项目负责，不能复用该单反射整机模型。
...
| `ACC-TF-002` | $E_{A1}=E_{A2}$ 退化 | $D_A=2(g_1+g_2-x_c)$ |
