...
接口就绪与质量过滤入口的独立粒子并发统一由仓库公共SIMION调度器决定，项目只提交粒子总数和完整
数值身份，不提交CPU、内存、安全余量或批次数。无相同身份历史时，首个正式批次即为资源识别批次：
最多观察30 s但不终止，随后保留其结果并重排剩余粒子；相同身份历史存在时直接按CPU与内存共同上限
生成最终批次。运行中的5 s错峰、内存暂停和持续临界内存下的末启动进程逐个回收同样只由公共调度器
执行。
