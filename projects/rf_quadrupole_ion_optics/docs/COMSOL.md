...
同日history快照追溯。
该诊断保留用于正式接口构建链在 `ReleaseFromDataFile` 节点初始化阶段中断、节点数量或输入绑定
异常时，以同一冻结 N=100 输入隔离重现，不用于性能筛选或替代粒子求解。退出条件是正式构建链已有
覆盖相同 release 节点、粒子初始化及失败证据的行为验证，且该诊断不再有独立消费者；届时连同入口、
专属测试和登记一并退役。当前仍满足独立诊断用途，保留活动实现。
