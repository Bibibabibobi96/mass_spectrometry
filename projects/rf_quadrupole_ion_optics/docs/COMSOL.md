...
- 无加速N=100基线已在四段杆实体几何上完成，RF-on为100/100、zero-RF为21/100；它只证明基线
  功能分类。基线pilot后、refined运行前登记的v2空间敏感性档为局部`0.5→0.35 mm`，时间档固定
  `0.35 mm`并将`80→160`步/周期；两档尚未运行，不得称为收敛。
