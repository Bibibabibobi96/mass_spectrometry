...
from projects.rf_quadrupole_ion_optics.analysis.validate_release_construction_gate import (
...
ARTIFACT_ROOT = REPO_ROOT.parent / "artifacts" / "projects" / "rf_quadrupole_ion_optics"
...
class ComsolWorkflowArchitectureContractTests(unittest.TestCase):
    def test_current_comsol_numerics_authorities_have_disjoint_workflow_scopes(
        self,
    ) -> None:
        candidates = sorted(PROJECT_ROOT.glob("config/*comsol*solver*numerics*.json"))
        self.assertEqual(candidates, [NUMERICS, MULTIPOLE_TRANSPORT_NUMERICS])
        ...
        multipole_contract = json.loads(
            MULTIPOLE_TRANSPORT_NUMERICS.read_text(encoding="utf-8")
        )
        self.assertEqual(
            multipole_contract["role"],
            "multipole_comsol_solver_numerics_profiles",
        )
        self.assertEqual(
            multipole_contract["project_id"], "rf_quadrupole_ion_optics"
        )
        runtime_profiles = json.loads(RUNTIME_PROFILES.read_text(encoding="utf-8"))
        self.assertEqual(
            runtime_profiles["solver_numerics_registry_paths"]["comsol"],
            "config/multipole_transport_comsol_solver_numerics.json",
        )
        runtime_numerics_ids = {
            profile["comsol_solver_numerics_profile_id"]
            for profile in runtime_profiles["profiles"].values()
        }
        self.assertEqual(
            runtime_numerics_ids, set(multipole_contract["profiles"])
        )
        ...
        for consumer in specialized_consumers:
            source = _read(consumer)
            self.assertIn("comsol_solver_numerics.json", source, consumer.name)
            self.assertNotIn(
                "multipole_transport_comsol_solver_numerics.json",
                source,
                consumer.name,
            )
    ...
    def test_active_profile_binds_numerics_identity_and_path_only(self) -> None:
        ...
        self.assertEqual(
            ...
            "projects/rf_quadrupole_ion_optics/config/comsol_solver_numerics.json",
        )
