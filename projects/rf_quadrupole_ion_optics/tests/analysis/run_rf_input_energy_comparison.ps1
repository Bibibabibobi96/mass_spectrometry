...
$runConfig=Join-Path $runDir 'run_config.json';[ordered]@{schema_version=1;run_id=$RunId;project='rf_quadrupole_ion_optics';mode='rf_to_oatof_input_energy_comparison';project_root=$repoRoot;inputs=[ordered]@{control_manifest=$controlManifestPath;candidate_manifest=$candidateManifestPath;control_events=$controlEvents;candidate_events=$candidateEvents;control_ion=$controlIon;candidate_ion=$candidateIon;contract=$contract;analysis=$analysis};parameters=[ordered]@{control_run_id=$ControlRunId;candidate_run_id=$CandidateRunId;particles=100;only_source_variable='kinetic_energy_eV';solver_rerun=$false};formal_gate_passed=$false}|ConvertTo-Json -Depth 6|Set-Content $runConfig -Encoding UTF8
