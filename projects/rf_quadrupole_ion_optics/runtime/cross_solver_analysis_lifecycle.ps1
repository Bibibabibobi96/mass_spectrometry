...
function Assert-CrossSolverSourceManifest {
    ...
    $null = & $Python (Join-Path $RepoRoot 'common\contracts\verify_run_manifest.py') $Manifest `
        --require-status success --require-project rf_quadrupole_ion_optics
...
function New-CrossSolverFrozenPathSet {
    ...
    [pscustomobject]@{
        ...
        core=Join-Path $moduleRoot 'projects\rf_quadrupole_ion_optics\analysis\particle_state_comparison_core.py'
