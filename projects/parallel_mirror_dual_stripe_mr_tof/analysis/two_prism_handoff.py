...
prism.  In the manufactured topology the ion must traverse the negative-side
Stripe bands after P2 before reaching the selected slow-drift phase origin:
the first negative mirror turn at ``y=0``.  Its ``z`` coordinate is observed
from the Stripe-on three-dimensional trajectory rather than imposed as a CAD
coordinate; the bare-mirror turning coordinate is only an analytic seed and
...
def audit_two_prism_voltage_definition(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    slow kinetic energy/direction, while the selected first negative mirror
    turn must occur at ``y=0``.  These are the two physical targets.  At that
    event ``v_z=0`` and ``z`` is an event-localized full-field result;
    ``x=0`` follows from symmetry and is not an independently steerable
    residual.
    """
    ...
    common = {
        ...
        "independent_target_conditions_before_finite_3d_jacobian": [
            f"first_negative_mirror_turn_project_y_equals_{entry_y:g}_mm",
            "negative_project_y_slow_kinetic_energy_from_the_compatible_Mirror_Stripe_solution",
        ],
        "event_or_symmetry_conditions_not_counted_as_voltage_residuals": [
            "negative_mirror_turn_has_v_z_equals_0_by_event_definition",
        ...
        "p2_exit_mechanical_acceptance": {
            ...
            "semantics": "a collision-free P2-exit bound only; the later drift-origin turn lies in the negative mirror",
        },
    }
    ...
    required = {
        "event": "first_negative_z_mirror_turn_after_P2_and_after_pre_origin_Stripe_traversal",
        "target_project_z_rule": "not_an_independent_target__observe_the_Stripe_on_full_field_turn__bare_mirror_turn_is_seed_only",
        "fast_velocity_condition": "v_z=0 with outward negative-z motion reversing toward positive z",
    }
    ...
    try:
        ...
        raise CandidateContractError("drift-phase-origin authority needs finite x/y targets") from error
    source = str(phase.get("source", ""))
    if not math.isfinite(target_y) or not math.isfinite(target_x) or target_y != entry_y or target_x != 0.0 or not source:
        raise CandidateContractError("drift-phase-origin authority conflicts with the project symmetry/Stripe origin")
...
class TwoPrismTransportObservation:
    """One collision-free source -> P1/P2 -> negative-turn trajectory."""
...
def observation_from_simion_events(
    ...
    *, ion_number: int = 1,
    ...
    """Extract the first post-P2 negative mirror turn used as drift origin."""
    ...
    p1 = [event for event in ion_events if event.get("kind") == "p1_plane" and event.get("n") == 1]
    phase_origins = [event for event in ion_events if event.get("kind") == "drift_phase_origin"]
    if len(p1) != 1 or len(phase_origins) != 1:
        raise CandidateContractError("P1/P2 hand-off needs exactly one forward P1 and one negative-turn phase origin")
    p1_state = _state_from_event(p1[0], "P1")
    origin_state = _state_from_event(phase_origins[0], "negative-turn phase origin")
    if origin_state.position_mm[2] >= 0.0 or origin_state.velocity_mm_per_us[1] >= 0.0:
        raise CandidateContractError("drift phase origin must be the outbound negative-z mirror turn")
...
def prism_turn_handoff_residuals(
    ...
    *,
    ...
    The first residual sets the actual negative mirror turn on the frozen
    ``y=0`` slow-drift origin.  The second sets the slow energy partition.  The
    ...
    return (
        ("P1_P2_first_negative_turn_y_mm", actual.position_mm[1] - target_y),
