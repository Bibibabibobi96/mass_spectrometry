...
try{
  ...
  $inputRoots=@($exactK,$workbench,$prewarm,$geometry)+$(if($useRealField){@($realFieldL1,$axisResponse)}else{@()})+$(if($useVoltagePoint){@($voltageCorrection)}else{@()})
  foreach($root in $inputRoots){& $python (Join-Path $repoRoot 'common\contracts\verify_run_manifest.py') (Join-Path $root 'run_manifest.json') --require-status success;if($LASTEXITCODE-ne0){throw "Input manifest failed: $root"}}
  $operatingCacheIdentity=Join-Path $prewarm 'results\local_operating_pa_cache_identity.json'
  ...
  $prewarmFinalProbePath=Join-Path $prewarm 'results\local_operating_pa_cache_final_probe.json'
  if(-not(Test-Path -LiteralPath $prewarmFinalProbePath -PathType Leaf)){throw 'Half-millimeter prewarm lacks its final frozen operating-PA probe receipt.'}
  ...
  if([string]::IsNullOrWhiteSpace($prewarmGenerationDirectory) -or -not(Test-Path -LiteralPath $prewarmGenerationDirectory -PathType Container)){
    throw 'Half-millimeter prewarm final receipt references no available frozen operating-PA generation.'
  }
  $pinnedGenerationSha256=Split-Path -Leaf (Resolve-Path -LiteralPath $prewarmGenerationDirectory).Path
  ...
  if([string]$prewarmFinalProbe.cache_key-ne[string]$operatingCacheProbe.cache_key){throw 'Half-millimeter prewarm final receipt and active operating-PA identity have different cache keys.'}
  $operatingCacheProbe=(@(Invoke-ProjectPython -Arguments @(
    '-m','projects.parallel_mirror_dual_stripe_mr_tof.analysis.local_operating_pa_cache',
    '--action','probe','--identity-input',$operatingCacheIdentity,'--cache-root',$operatingCacheRoot,
  ...
  [int64]$headroom=16GB
  $startup=Invoke-ArtifactCapacityGate -Python $python -RepoRoot $repoRoot -ArtifactRoot $artifactRoot -RequiredHeadroomBytes $headroom -ProtectedPaths (@($package.artifact_run_dir)+$inputRoots) -ProtectedCacheKeys @($operatingCacheKey)
  ...
  Write-RunJson -Path $parentGenerationContract -Depth 30 -Value ([ordered]@{
    ...
    prewarm_run_manifest_sha256=(Get-FileHash -LiteralPath (Join-Path $prewarm 'run_manifest.json') -Algorithm SHA256).Hash
  ...
  $familyContracts=@();$fixedCacheReceipts=@();$quarter=@{};$fixedMisses=@();$fixedCacheKeys=@();$fixedResourceProfile=$null;$fixedFormalRecord=$null
  ...
  foreach($entry in @(@('negative_mirror','mirror_turn_negative','local_negative_mirror.pa'),@('positive_mirror','mirror_turn_positive','local_positive_mirror.pa'))){
    ...
    if([string]$probe.disposition-eq'hit'){
      $cacheResult=(@(Invoke-ProjectPython -Arguments (@($cacheArguments)+@('--action','materialize','--destination-directory',$fixedDir)))-join"`n")|ConvertFrom-Json -Depth 30
  ...
  if($fixedMisses.Count-gt0){
    ...
    $fixedCapacity=Invoke-ArtifactCapacityGate -Python $python -RepoRoot $repoRoot -ArtifactRoot $artifactRoot `
      -RequiredHeadroomBytes ($headroom+$fixedTransientBytes) -ProtectedPaths (@($package.artifact_run_dir)+$inputRoots) `
    ...
    foreach($miss in $fixedMisses){
      ...
      $null=Invoke-ProjectPython -Arguments (@($miss.cache_arguments)+@('--action','materialize','--destination-directory',$fixedDir))
      ...
      $fixedCacheReceipts+=$cacheReceipt;$quarter[$miss.label]=$miss.destination
