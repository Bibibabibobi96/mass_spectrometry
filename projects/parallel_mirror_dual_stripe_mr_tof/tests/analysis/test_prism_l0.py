...
class FirstPrismL0Test(unittest.TestCase):
    ...
    def test_derives_total_energy_partition_triangle_crossings_and_static_seed(self) -> None:
        ...
        self.assertEqual(result.total_kinetic_energy_ev, 4000.0)
        self.assertEqual(result.drift_kinetic_energy_ev, 5.0)
        self.assertEqual(result.fast_kinetic_energy_ev, 3995.0)
        self.assertAlmostEqual(result.drift_angle_degrees, 2.0261339721915568)
        ...
        self.assertAlmostEqual(result.target_unit_direction_project[1], -(5.0 / 4000.0) ** 0.5)
        self.assertAlmostEqual(result.target_unit_direction_project[2], -(3995.0 / 4000.0) ** 0.5)
        ...
        self.assertAlmostEqual(result.hard_boundary_seed_voltage_v, 141.3329402510257)
    ...
    def test_rejects_energy_semantics_angle_convention_and_second_prism_premature_solution(self) -> None:
        for mutate in (
            ...
            lambda contract: contract["prism_transport"]["energy_partition"].__setitem__("total_kinetic_energy_ev", 4005),
    ...
    def test_contract_energy_and_target_plane_are_derived_not_code_constants(self) -> None:
        contract = copy.deepcopy(self.contract)
        contract["nominal"]["energy_per_charge_v"] = 4100
        contract["prism_transport"]["energy_partition"]["total_kinetic_energy_ev"] = 4100
