...
class MirrorRealFieldProfileTest(unittest.TestCase):
    ...
    def test_fixed_quarter_mm_runner_refines_only_two_operating_points(self) -> None:
        ...
        self.assertIn("Assert-FrozenOutputRecord", source)
        self.assertIn("prewarm_operating_pa_generation_projection.json", source)
        self.assertIn("source_manifest_selected_outputs_plus_current_complete_generation_probe", source)
        self.assertIn("half_mm_prewarm_run_manifest.json", source)
        self.assertIn("half_mm_operating_pa_cache_identity.json", source)
        self.assertIn("half_mm_operating_pa_cache_final_probe.json", source)
        self.assertIn("expectedRecordedGeneration", source)
        self.assertIn("[Parameter(Mandatory)][string]$PreparedAnalyzerSourceRunPath", source)
        self.assertIn("prepared_analyzer_source_receipt.json", source)
        self.assertIn("'--prepared-source-receipt',$preparedReceipt", source)
        self.assertIn("Get-VerifiedRunManifestInputRecord", source)
        self.assertIn("reviewed_mrtof_analyzer.gem", source)
        self.assertNotIn("@($exactK,$workbench,$geometry)+", source)
        self.assertIn("$capacityProtectedRoots=@($exactK,$workbench)", source)
        self.assertIn("--expected-generation-sha256',[string]$cacheResult.generation_sha256", source)
        self.assertIn("materialization generation differs from publication", source)
        self.assertEqual(source.count("'--action','probe','--identity-input',$operatingCacheIdentity"), 1)
        self.assertNotIn("half_mm_prewarm_manifest=Join-Path $prewarm 'run_manifest.json'", source)
        self.assertIn("Frozen half-millimeter operating PA generation is unavailable", source)
    ...
    def test_fixed_mirror_cache_misses_use_repository_scheduling_and_serial_publish(self) -> None:
        ...
        preflight = source.index("try{")
        self.assertLess(source.index("$fixedFormalRecord=$null"), preflight)
        self.assertLess(source.index("$fixedResourceProfile=$null"), preflight)
