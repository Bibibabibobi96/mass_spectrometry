...
test('negative turn is phase origin and same-side turn completes a cycle', function()
...
test('positive slow velocity closes at same negative-turn phase', function()
  local c, step, events = driver()
  ...
  assert(found.y_mm == 0.25 and found.vy_mm_us == 1)
end)

test('twenty-six negative-turn returns have no hidden K limit', function()
  local c, step = driver()
  arm_and_origin(step)
  for cycle = 1, 26 do
...
test('central crossings are diagnostics and never define phase', function()
  ...
  assert(c:state().cycles == 0 and c:state().phase == 'awaiting_positive_turn')
end)

test('armed origin must be an outbound negative mirror turn', function()
  local c, step = driver()
