...
按问题定位：[单个有限偏压 Stripe 的硬边界精确响应](#3-单个有限偏压-stripe-的硬边界精确响应) · [单个 Stripe 的空间–时间响应比](#7-单个-stripe-的空间时间响应比) · [标称点的闭式形状反演](#8-标称点的闭式形状反演) · [响应矩阵和条件数](#10-响应矩阵和条件数) · [基线宽度的全局影响](#11-基线宽度的全局影响) · [最低参考测试](#24-最低参考测试)。
...
作近似分类。设计中心仍须满足未取整的目标关系，并由真实转折事件与共同相位验证；
四舍五入后的 K 相同不能代替中心残差闭合，详见[原漂移返回定义](adiabatic_drift_and_original_ion_foil.md#4-漂移反转和返回时间)。
...
不产生 $y$ 向力，但会改变完整分析器周期和能量聚焦，因此须进入系统响应与全局校准。
这不允许用 Stripe 电压反求当前固定镜的独立镜理论解。
...
这里还有一个必须失败关闭的结构限制：不能把原论文的高次分量
$p_s$ 和线性/倾斜分量 $p_m$ 原样各交给一条静电 Stripe。原目标
...
会要求高次响应 $h_s=+1$、线性响应 $h_m=-1$。但可穿越的静电 Stripe
恒有 $h_i>0$；而 $h_s=1$ 只对应 $v_s=0$，此时有限宽度 Stripe 的作用量
扰动也为零。因此“一个纯高次曲线 + 一个纯线性曲线”不能精确实现该
论文目标。双 Stripe 非倾斜设计必须在选定两个不同的正 $h_i$ 后使用下节
的闭式反演重新生成**两条** $p_i$；一般而言，两条物理曲线都会包含共享的
高次部分，不能再分别解释为原论文的 $p_s$ 与 $p_m$。若保留现有纯高次/
纯线性硬件，只能另行定义并验证其可实现的 $\psi/g$ 目标，不能声称它精确
等同于原论文分解。

这个结论的范围只是“逐项仿真原论文的两个响应分量”。它不能直接关闭一套
固定曲线对论文**积分条件**的实例求解：后者应由真实 $S_1(y),S_2(y)$、两个
偏压和 $L$ 共同生成 $\psi,g$，再以空间返回、时间平台、完整周期残差及其
Jacobian 秩判定相容性。参考分量不等价与活动硬件无解是两个不同命题。
...
以下两种方式适用于允许联合改变镜设计的研究方案。当前制造硬件按 PROJECT 的有向求解关系
先取得独立镜 receipt，再校正下游；本节不能作为反向拟合镜电压的授权。
...
以下是新设计的表达示例，不是当前制造硬件配置或必建 Schema；数值取自原论文参考组。
