...
本项目把镜设计和整机终端校准分成两层。第一层先在中央 `z=0` 参考面求镜本体基线解，验证三点能量
等时、横向稳定、$\gamma$ 和电源约束；这是最简单、可解释的理论起点。第二层在几何、加速器、Stripe、
P1/P2、目标 $K$、源和探测器都冻结后，只沿该镜自身 Jacobian 给出的 TE1 方向做邻域微调，使真实探测
面上的一阶到达时间色散为零。此时 `z=0` 仍是加速器交接面和镜周期诊断参考面，但不再被当作不可移动的
最终时间焦面。TE1 微调必须继续满足镜三点等时预算、稳定性、$\gamma$、供电和既有返回拓扑；若不能
同时满足，应判定该邻域没有合格解，而不是牺牲镜本体资格来换取一个窄峰。
