...
按问题定位：[三点能量等时性](#3-三点能量等时性) · [Berdnikov 平面镜解析势](#4-berdnikov-平面镜解析势) · [周期积分的数值稳定性](#5-周期积分的数值稳定性) · [横向稳定性](#7-横向稳定性) · [运行校准接口 TE1/TE2](#12-运行校准接口-te1te2) · [最低参考测试](#15-最低参考测试)。
...
| 2 | 工作点相关；本页公开系数示例为负值 | 塑造反射势的低级段 |
| 3 | 公开示例为正电位 | 调整能量等时平台和像差 |
| 4 | 高于目标反射能量的电位 | 形成最终反射势垒 |

上表说明原论文模型，不规定本项目 B—E 的单调顺序；电位约束按目标几何和机器合同确定。
负离子工作时，所有电压极性反转。

当前已加工机器的单镜电源通道还给出独立于解析理论的硬包络：B、E 各使用一路
$\pm10\,\mathrm{kV}$ 电源，C、D 各使用一路 $\pm5\,\mathrm{kV}$ 电源；左右镜按同名电极组保持
对称电压，A 接地。求解器必须逐电极取该硬件包络与能量反射、B—D 动态上界等物理条件的交集。
这些额定值是机器合同，不是 Berdnikov 方程推导出的数值，也不能通过优化器容差越过。
...
若用有限差分计算 $T'(w)$，应扫描多组 $\Delta w$；下列仅为步长示例，实际范围由问题尺度与收敛确定：
...
| 线性稳定 | 全能量包络内 $\lvert(x\mid x)\rvert<1$ 且有裕量 |
