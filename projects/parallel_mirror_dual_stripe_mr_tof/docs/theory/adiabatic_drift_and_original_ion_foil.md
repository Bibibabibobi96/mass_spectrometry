...
对于名义中心粒子，设计方程是更强的精确中心条件

$$
\frac{T_D(\vartheta_0)}{T_0}=K.
$$

解析设计应直接解这条等式；有限场数值计算则必须另行声明由数值误差预算导出的残差容差。不能以
“四舍五入后仍等于 $K$”代替中心等式。

为避免不同 $K$ 的离子形成多重谱峰，指定为同一目标 $K$ 的**整个接受相空间**至少应满足
...
这里的两个边界都是严格边界；工程工作点还应为源展宽、边缘场和数值误差保留正裕量。该不等式是
整数振荡数/相邻 overtone 的归类条件，不是几何束斑“不重叠”的充分条件，也不单独证明空间返回、
时间聚焦或质量分辨率。中心粒子位于区间内仍不足够，必须对完整接受束团逐粒子检查。实际还必须检查：
...
对具有相同注入角和相同转折点的两粒子，若初始坐标分别为 $+y_0$ 和 $-y_0$，它们在理想慢漂移模型的
完整去程–返程中经历互为时间反演的路径。因此时间偏移是 $y_0$ 的偶函数：
...
这个偶函数关系不表示单个离子的整条空间轨迹必须“关于 $y$ 轴对称”。项目中的三种对称必须分开：

- 对称的电极与电压使 $x=0$ 成为横向不变平面；
- 对称的平行双镜使分析器内的快速反射动力学关于中央面 $z=0$ 对称；
- $y$ 向去程和返程只要求关于慢漂移转折时刻的时间反演关系，转折位置通常为 $y=y_D$，而不是要求
  几何轨迹关于坐标轴 $y$ 镜像。

完整仪器中的加速器、P1/P2和探测器还会破坏全局空间对称，因此这些理想关系必须在实际路径事件上分别验证。
