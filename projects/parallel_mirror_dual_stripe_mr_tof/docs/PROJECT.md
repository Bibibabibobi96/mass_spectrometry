...
$L,W,\kappa(1),\vartheta_0$ **不是四个可分别指定的输入**：它们必须在同一镜—Stripe—漂移联合
L0/L1 求解中派生。特别是 Stripe 的 $S_0$ 基线会改变共同周期和作用量能量导数，故禁止把孤立镜的
`reduced_period` 或由其得到的 $W$ 当作固定 Stripe 输入，再反向施加 $K$ 或入口角；这会过定义并可能
产生彼此矛盾的数值。

对当前**几何已加工冻结**的镜，电压仍可调，故 $W$ 是镜电压解族的一项自由派生量；固定的双 Stripe
曲线、目标返回和 $K$ 共同筛选该自由度。联合求解器可以用 $W$ 参数化这族镜电压解以改善数值条件，
但同一次残差必须重新计算其全周期 $W$，并强制两者相等，不能将“指定的 $W$”和“电压得到的 $W$”
算作两条独立物理约束。未来新硬件设计可反过来把 $W$ 作为显式设计指标，联立镜几何、镜电压和 Stripe
形状求解；这属于另一种设计自由度集合，不能倒灌到当前加工件合同。
...
$W=R\sqrt{E/q}$ 仅在**不含 Stripe 作用量的镜子诊断**中成立，不能单独形成 Candidate receipt。正式
联合残差须用包含双 Stripe 基线作用量的全周期；在该解同时满足三点等时、Poincare 稳定/$\gamma$、
$\psi/g$、端点正则化空间返回与目标 $K$ 前，P1/P2 入口相空间继续保持未解。

最终闭环的输入是冻结的离子源位置、方向和在 P1/P2 处的总动能，以及固定 CAD/坐标变换；待求量仅为
Mirror B--E、两组 Stripe 和按飞行顺序 P1(ID 16)、P2(ID 17) 的八个电压。Stripe 入口的位置、方向、
名义转折点、$L$、$W$、$\kappa(1)$ 和 $\theta_0$ 都是同一试验的导出量，绝不能倒置成外部拟合坐标。
三维 P1/P2 输运必须将源射线送到该试验理论要求的 Stripe 入口点和方向；这两个失配与 Mirror--Stripe
残差一起组成完整 Jacobian。只有其实际秩和增广秩表明系统恰定，才可自动发布全部八个电压；欠定或不相容
时只报告诊断，绝不以扫描、默认电压或固定入口条件伪造解。
