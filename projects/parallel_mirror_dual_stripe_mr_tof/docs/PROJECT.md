...
联合计算在发布任何数值前还必须报告**定义性判据**：列出物理未知量、具名残差和该点的残差 Jacobian
独立秩。独立秩低于未知量数即欠定义；增广残差秩揭示不相容时即过定义；只有独立秩恰好闭合全部物理
自由度时，才可由给定量导出其余 $L,W,\kappa,\vartheta_0$ 或棱镜状态。仅比较“方程条数 = 参数条数”
不构成恰好定义的证据，冗余/相关约束和矛盾约束都必须显式暴露。
...
理论链分为两个有向问题。第一题仅以固定镜几何和镜的能量条件求 Mirror B--E；镜周期/W 是其输出 receipt。
第二题以该 receipt、冻结离子源位置/方向/总动能和 CAD/坐标变换为输入，求两组 Stripe 及路径顺序
P1(ID 16)、P2(ID 17) 的四个电压。Stripe 入口的位置、方向、名义转折点、$L$、$\kappa(1)$ 和
$\theta_0$ 是第二题的导出量，绝不能倒置成外部拟合坐标。三维 P1/P2 输运必须将源射线送到理论要求的
Stripe 入口点和方向。两个问题都必须分别以实际 Jacobian 与增广秩判定恰定；欠定或不相容时只报告诊断，
绝不以扫描、默认电压或固定入口条件伪造解。
...
首个按该权限划分发布的双Stripe解析种子是`20260907_192000__analysis__python__dual-stripe-operating-seed`。搜索使用30个确定性起点，其中21个入口可行、代理函数发现2个根族；原生B-spline精化后，近单-Stripe分支被无量纲双响应门禁拒绝，保留的非退化种子为`[v1,v2]=[-44.2474373022,+120.108072884] V`。在当前合同`K=25`下，它给出`L=306.247900921 mm`、镜收据保持`W=540.449945808 mm`、`kappa(1)=1.55983037118`及名义注入角`2.02613397219 deg`；`K`残差为`1.96e-11`，数值`kappa-prime`残差为`-3.56e-8`。这些都是当前实例结果，不要求等于论文印刷数值。四个用户节点的`tau_g`导数仍为`1.247--1.902`，完整分析器高低能相对周期残差约`+6.19e-7/-6.18e-6`，所以该run仅为`analytic_seed_only__time_platform_energy_and_P1_P2_pending`，不能直接写入最终SIMION电压表。早一条88点诊断run `20260907_190500__analysis__python__dual-stripe-operating-seed`只保留了近单-Stripe分支，已被本run的原生span两倍采样和双响应门禁取代，不得消费。
