...
2026-09-08 用户进一步澄清当前已加工拓扑：P2 后的离子必须先穿过负侧物理 Stripe 带区，不能把中央
grounded-2 走廊当作 P2 到 Stripe 的直接入口。慢漂移相位原点现定义为该预入口穿越之后的**首个负侧镜
转折点**：$y=0$、$v_z=0$、$v_y<0$。Stripe 开启后真实转折的 $z$ 由完整三维场轨迹事件定位，不是第三个
棱镜目标；裸镜势、轴向能量和镜电压收据给出的 $-z_t(E)$ 只作解析初值和偏差诊断，禁止写死 CAD 坐标。
完整漂移结束时要求回到同一个负侧镜转折相位，$y=0$ 且 $v_y>0$。

因此 P1/P2 的两个物理目标改为：首个负镜转折的 $y=0$ 位置，以及相容 Mirror--Stripe 解给出的负向慢
动能/速度。转折事件本身给出 $v_z=0$，实际 $z_t$ 是 Stripe-on 三维轨迹结果，$x=0$ 则是对称不变量；三者均不重复
计作可调残差。中央 grounded-2 十字孔的 $z\in(-40,40)\,\mathrm{mm}$ 仅约束 P2 出口可通行性，
不能约束稍后位于负镜内的漂移相位原点。两个目标已经在物理上声明，但 P1/P2 电压仍须由有限三维轨迹的
尺度化二乘二 Jacobian 证明满秩和局部相容后才能发布。
...
合同审计接受事件定位的负镜转折规则而不接受固定 `target_project_z_mm`；下一份受管三维 Jacobian 收据形成前，
...
累计快相位、同一负镜转折相位返回以及 P1/P2 有限三维输运后才能发布；裸镜归一化
...
receipt，生成 Stripe 种子及 $y=0$ 负镜转折目标态，最后由 source/focus--P1--P2--Stripe--负镜转折的
...
对当前冻结的 Stripe 形状，两条有效 $S_i(y)$ 曲线均以项目坐标 $y=0$ 为理论端点，且 $S_i(0)$ 必须进入
基线作用量和完整分析器周期。由于已加工拓扑迫使离子在到达该慢漂移原点前先穿越负侧 Stripe 带区，
`y=0` 不再解释为 P2 出口或“第一次进入 Stripe”的平面；它与首个负侧镜转折事件共同定义漂移相位原点。
CAD 在 $y=[0,2]$ 的平面端部只是机械连续段，不能当作 $S_i$ 的外推或新的理论原点。

数值验证顺序固定为：先用单个中心离子检查 P1/P2 后先穿越 Stripe、首个负镜转折落在 $y=0$、无撞电极、
每次双镜 $z$ 转折、Stripe 慢漂移转折、同侧负镜转折周期、裸镜关断 Stripe 后得到的 $T_0/W$、以及
...
另有纯Lua周期状态机测试通过；该模块现以首个`v_y<0`负侧镜转折为主漂移相位原点，以随后同侧负镜
转折闭合一个完整快周期，并在首个`v_y>0`负镜转折记录返回`y`和实际K。`z=0`中央面仅作诊断，P1
