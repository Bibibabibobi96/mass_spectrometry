...
首棱镜现有独立的 solver-neutral L0 几何审查合同：其参考总动能为 `4000 eV`，历史 `5 eV`
慢漂移动能和`2.026133972°`方向只用于已存在的首棱镜静态种子及完整 Stripe 搜索的起点生成，
不再是活动工作点约束。CAD 三角棱镜 ID 16 的硬边界种子在合同指定的局部角度约定下为
...
解析入口角必须同时满足转折能量式
$\sin\vartheta_0=\sqrt{[\Phi(L)-\Phi(0)]/w_0}$ 与项目理论式
$\vartheta_0=\arcsin[\kappa(1)L/(KW)]$；二者之差由目标 K 残差显式检查。公开 Astral 的
`1.784°/335 mm/641 mm` 仅用于函数回归，不能写入本 Candidate 工作点。这里的
$L,\kappa(1),\vartheta_0$ 不是可分别指定的输入；它们由下游 Stripe 漂移问题派生。$W$ 则先由
**独立的镜理论**确定：固定镜几何上的 Berdnikov L0/L1 仅用镜自身的三点等时、转折、Poincare 稳定与
$\gamma$ 条件求 B--E，并发布镜周期/W receipt。Stripe 的 $S_0$ 基线可改变完整分析器的飞行周期与
...
第二题对每个合格镜根以两组 Stripe 电压和固定曲线有效区间内的物理漂移长度 $L$ 为三个求解坐标；
在该物理转折归一化使 $\psi(1)=1$ 恒等成立，再同时检查 $K$、$\kappa'(1)$、四节点时间平台和三个
完整分析器局部能量斜率。`5 eV`只生成数值起点，不参与最终残差。候选的入射角、慢漂移动能
$w_y=w_0\sin^2\vartheta_0$ 和快反射能量 $w_z=w_0-w_y$ 均由同一解派生；若用户以后明确把 5 eV
提升为设计条件，必须把它作为新增具名等式重新做定义性审计，而不能静默固定。
第三题才以冻结源状态、第二题导出的 Stripe 入口状态及 CAD/坐标变换为输入，求路径顺序 P1(ID 16)、
P2(ID 17) 的两个电压。棱镜电压不得混入第二题来掩盖固定 Stripe 曲线的不相容。Stripe 入口的位置、
方向、名义转折点、$L$、$\kappa(1)$ 和 $\theta_0$ 都是导出量，绝不能倒置成外部拟合坐标。三维 P1/P2
...
固定硬件曲线、同一组 Stripe 电压和合同引用的能量微分步长。它们是对当前实例系数的相容性检查，
不要求当前 $L/W$ 或曲线系数等于论文样机的印刷值。
...
电压。此分段只用于给各自的非线性求解器一个物理起点，不会把中间结果升格为最终解。镜 B--D 的硬件电压包络为 $[-10,10]\,\mathrm{kV}$；
E 同样不超过 $+10\,\mathrm{kV}$，但其闭合外端板电压必须严格高于该镜支路的最大 $E/q$ 才能反射。
...
最新的固定几何工作流不再从单一起点直接联立猜测：L0用36条全包络随机重启只解3900/4000/4100 V三条周期斜率等式，并保留全部合格族成员；L1先筛查稳定映射，再以E为连续参数跟踪该三等式解族并寻找`gamma=90°`交点。外层标量求根后必须再进行四变量交点细化，其中前三个残差仍是L0三等式，第四个只作为L1族成员选择条件；因此不会把`gamma`误记为第四条L0方程。合同要求`|gamma-90°|<=0.001°`，并以`0.004/0.008/0.016 mm`及`40/80/160 urad`三档有限差分探针对`gamma`和`T̄_xx`做收敛门禁。E只被要求严格高于最高能量节点4100 V且不超过10 kV；B/C/D同样只受`±10 kV`包络约束，不存在“只有E可高于3900 V”或B--E单调性的附加条件。实际首个上升支路转折连续性、稳定性、峰值场、三维无碰撞和SIMION网格收敛才是后续淘汰条件。
...
固定CAD双Stripe曲线的首个求解器无关结构辨识也已闭合到诊断级，当前受管证据为`20260907_183500__analysis__python__dual-stripe-paper-structure-authority`：快速评价器直接编译同一原生B-spline knot/control合同，并与原递归权威评价在抽查点保持约`1e-13 mm`一致，不生成采样形状或第二几何真值。按照用户确认，当前曲线只要求保持“set-1无常数五次多项式分量、set-2无常数一次分量”的结构，不假定其系数等于论文印刷值。对完整`y=[-390,0] mm`跨度按合同的`1/2/4`倍采样拟合物理距离多项式，最高档RMS分别为`0.00157089 mm`和`0.000300643 mm`；物理坐标系数及逐档残差由机器报告直接给出。该结构本身不能唯一识别`L`：任意正`L`都可由`c_k(L)=b_k L^k`把同一物理多项式改写到`eta=y/L`，再叠加归一化尺度自由度。因此此前强制套用论文印刷系数得到的约`340.018 mm`只能作为被撤回的参考回归，不能成为当前CAD的`L`、节点跨度或电压输入。`L`必须由镜收据、Stripe作用量/归一化、目标K和P1/P2入口状态共同闭合；在此以前四个eta节点的物理跨度保持pending。

用户进一步明确：本项目活动模型的理论方程、无量纲多项式加一次项结构，以及作用量、赝势、`kappa`、`tau_g`、`K`、`L`、`W`和注入角之间的关系均与论文模型一致；当前硬件实例允许不同的是多项式/一次项系数、`L`、`W`和电压等数值解。上一版结构诊断曾把“几何高次分量/几何一次分量”未经当前实例归一化和电压响应求解便逐项等同于`psi_s/psi_m`，进而给出“不相容”判断；该推断越过了现有证据，已撤回，旧run `20260907_172342__analysis__python__dual-stripe-shape-structure`中的对应字段不得消费。结构拟合现在只确认函数族，不判断完整论文方程的相容性；相容性必须用当前系数、同一镜收据和完整下游Jacobian实际计算。
