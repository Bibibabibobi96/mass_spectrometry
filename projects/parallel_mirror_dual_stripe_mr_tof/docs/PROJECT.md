...
4000 eV 镜根曾暴露出与目标 `K=25` 不相容：由 `5/4000 eV` 正交分量、`L=340 mm` 和该镜的
`W=586.9393397 mm` 得到 `T_D/T_0=24.4153484`。这已由新的系统级能量选择 supersede，而不是靠
...
`T0=29.966365574 us`。程序按用户指定的四个时间平台节点求得活动无量纲目标
`kappa(1)=1.49406572347`，最终 `T_D/T_0=25.0000000000124`，K 残差
`1.24e-11`，远低于保持不变的 `1e-6` 解析 Candidate 门槛。三条局部归一化周期斜率的最大绝对值为
`1.2713e-9 V^-1`，`gamma=89.9999999961 deg`，三档 L1 探针收敛通过。局部五乘五缩放雅可比为满秩、
`nullity=0`、`status=square_exact`，故在当前声明的未知量和约束下恰好定义；其条件数约
`3.73e5`，提示后续必须报告电压、几何和三维场敏感性。该回执仍只是 solver-neutral 二维 Candidate，
不是 SIMION 电压权威；完整三维接受束团仍须逐粒子验证处于 `(24.5,25.5)`，且半整数区间只用于
...
`[v1,v2]=[-49.637405105,+99.752949296] V`，其连续中心计数为
`25.0000000000262`（残差 `2.62e-11`）。这只是 exact-K-consistent 的解析 Stripe 初值；P1/P2、有限三维
