...
固定CAD双Stripe曲线的首个求解器无关结构辨识也已闭合到诊断级，受管证据为`20260907_172342__analysis__python__dual-stripe-shape-structure`：快速评价器直接编译同一原生B-spline knot/control合同，并与原递归权威评价在抽查点保持约`1e-13 mm`一致，不生成采样形状或第二几何真值。按照用户确认，当前曲线只要求保持“set-1无常数五次多项式分量、set-2无常数一次分量”的结构，不假定其系数等于论文印刷值。对完整`y=[-390,0] mm`跨度按合同的`1/2/4`倍采样拟合物理距离多项式，最高档RMS分别为`0.00157089 mm`和`0.000300643 mm`；物理坐标系数及逐档残差由机器报告直接给出。该结构本身不能唯一识别`L`：任意正`L`都可由`c_k(L)=b_k L^k`把同一物理多项式改写到`eta=y/L`，再叠加归一化尺度自由度。因此此前强制套用论文印刷系数得到的约`340.018 mm`只能作为被撤回的参考回归，不能成为当前CAD的`L`、节点跨度或电压输入。`L`必须由镜收据、Stripe作用量/归一化、目标K和P1/P2入口状态共同闭合；在此以前四个eta节点的物理跨度保持pending。

该辨识同时给出一个与具体多项式系数无关、但只在“强制复现原倾斜镜目标”时适用的解析否定结果：当前固定分量映射是set-1纯`psi_s`多项式、set-2纯`psi_m`一次项。若强制精确复现`psi=psi_s+psi_m`及`g=psi_s-psi_m`，匹配空间响应后必需`h_high=+1`、`h_linear=-1`；但常偏压Stripe的`h=sqrt(w0/(w0-v))`在实数透射域严格为正，且`h=1`只对应零偏压、没有独立作用量响应。因此这两条纯分量CAD曲线不能用两个常偏压精确恢复原方案的完整`psi/g`目标。该结论不否定对当前加工件重新定义自洽的双Stripe目标；后续应以实际曲线系数、镜收据和用户指定节点做秩/相容性分类，或建立显式近似/完整三维性能目标并报告不相容残差，不得用加权最小二乘伪装成恰好定义的精确解。
