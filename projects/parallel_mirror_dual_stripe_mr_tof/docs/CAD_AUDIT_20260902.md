...
of the three ion-foil files directly in SolidWorks 2022 returned error 2.
Therefore their curve, thickness, and transform are unresolved.
...
It must not emit a purported CAD-faithful curved Stripe, global mirror spacing,
or prism placement until a read-only audit can recover the ion-foil parts and
the relevant subassembly transforms. A repaired *working copy* may be used for
that recovery only after the source archive remains immutable and the repair
mapping is independently documented.
