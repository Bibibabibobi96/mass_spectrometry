...
required = {'particle_id','frame_id','clock_epoch_id','instrument_time_us', ...
    ...
    'parent_particle_id','generation','species_id','particle_weight','phase_reference_id', ...
    'lineage_birth_time_us','particle_birth_time_us', ...
    'mass_to_charge_Th', ...
...
canonicalParentIds = strings(height(ions), 1);
for parentIndex = 1:height(ions)
    canonicalParentIds(parentIndex) = canonical_parent_particle_id( ...
        ions.parent_particle_id, parentIndex);
end
...
exitRows = cell(height(ions), 28); exitCount = 0;
for index = 1:height(ions)
    ...
    if activeAtPulse
        ...
        captureRows(captureCount,:) = {ions.particle_id(index), ...
            string(ions.frame_id(index)),string(ions.clock_epoch_id(index)),pulseState.t_s*1e6, ...
    ...
    if exited
        ...
        exitRows(exitCount,:)={ions.particle_id(index),canonicalParentIds(index), ...
            ions.generation(index),string(ions.species_id(index)),ions.particle_weight(index), ...
            'rf_quadrupole_to_oatof_s3', ...
            ...
            energyEv,string(ions.phase_reference_id(index)), ...
            mod(2*pi*frequency*state.t_s+phase,2*pi)};
    end
end
...
capture=cell2table(captureRows(1:captureCount,:),'VariableNames',{'particle_id','frame_id', ...
    'clock_epoch_id','instrument_time_us', ...
...
localExit=cell2table(exitRows,'VariableNames',{'particle_id','parent_particle_id','generation', ...
    'species_id','particle_weight','source_component_id','target_component_id','state_event', ...
    ...
    'kinetic_energy_eV','phase_reference_id','phase_rad'});
end

function parent = canonical_parent_particle_id(column, index)
if isnumeric(column)
    value = column(index);
    ...
    token = strtrim(string(column(index)));
    if ismissing(token) || strlength(token) == 0 || strcmpi(token, "NaN")
        parent = "";
        return
    end
    value = str2double(token);
end
assert(isfinite(value) && value >= 1 && value == fix(value), ...
    'parent_particle_id must be empty or a positive canonical integer.');
parent = string(sprintf('%.0f', value));
end
