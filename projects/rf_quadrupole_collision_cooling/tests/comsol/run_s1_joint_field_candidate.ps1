param(
  ...
  [double]$OuterVacuumMarginMm = 1.0,
  [string]$RunId = ''
)
...
if (-not [bool]$jointSourceDocument.permissions.field_solve_allowed) {
  throw 'S1 joint-field solve is blocked until the continuous RF shield radius is selected and validated.'
}
...
$allowedMargins = @($jointSourceDocument.local_domain.outer_vacuum_margin_diagnostic_mm | ForEach-Object { [double]$_ })
if (-not ($allowedMargins | Where-Object { [math]::Abs($_-$OuterVacuumMarginMm) -le 1e-12 })) {
  throw "OuterVacuumMarginMm must match a frozen S1 value: $($allowedMargins -join ', ')"
}
if ([string]::IsNullOrWhiteSpace($RunId)) {
  ...
  if ([math]::Abs($OuterVacuumMarginMm-1.0) -gt 1e-12) { $detail += ('margin' + ([string]$OuterVacuumMarginMm).Replace('.','p')) }
...
if (-not (Test-Path -LiteralPath $closedReference -PathType Leaf)) { throw 'Frozen closed-shield field sample is missing.' }
...
[ordered]@{
  ...
  parameters=[ordered]@{geometry_state=if([math]::Abs($PortWidthMm) -lt 1e-12){'closed_local_domain_control'}else{'opened_port'};joint_scope=$JointScope;port_full_width_y_mm=$PortWidthMm;port_full_height_z_mm=0.9;downstream_buffer_after_grid2_mm=$DownstreamBufferMm;outer_vacuum_margin_mm=$OuterVacuumMarginMm;mesh_auto_level=$MeshAutoLevel;accelerator_hmax_mm=$AcceleratorHmaxMm;release_volume_hmax_mm=0.1;solver_rerun=$true;particle_tracking=$false;model_saved=$false}
...
try {
  try {
    ...
    $env:RF_OATOF_OUTER_VACUUM_MARGIN_MM=[string]$OuterVacuumMarginMm
  ...
  & $python $analysis --candidate $fieldCsv --closed-reference $closedReference --joint-contract $joint --interface-contract $interface --output-dir $resultDir
