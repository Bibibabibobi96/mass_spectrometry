...
sourceCenterMatches = all(abs(sourceCenter(:)- ...
    registration.source_exit_center_instrument_mm(:)) <= 1e-12,'all');
targetCenterMatches = all(abs(targetCenter(:)- ...
    registration.target_entry_center_instrument_mm(:)) <= 1e-12,'all');
gapVectorMatches = all(abs(targetCenter(:)-sourceCenter(:)-gapMm*sourceAxis(:)) <= 1e-12,'all');
assert(sourceCenterMatches && targetCenterMatches && gapVectorMatches, ...
