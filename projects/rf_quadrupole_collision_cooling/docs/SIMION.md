...
无碰撞回归固定`official_transport`身份。三个入口均不接受`Mode`或按文件扩展名自动选择用途。
...
当前PA杆区已有细化参考，但入口/出口边缘仍有离散敏感性，不能把杆区收敛外推为全域收敛。轴向加速
的分段杆、出口带孔接口板和显式多级工况均已有N=100功能证据；它们不代表PA收敛、与COMSOL数值等价或机械
资格。接口N=100双端100/100但相空间严格比较为FAIL。

RF→oaTOF下游由本项目累积S3入口驱动，以COMSOL真实局部出口canonical状态进入只读oa-TOF分析器；
