...
RF+DC质量过滤已完成求解器无关有限长度L1功能扫描。L1使用当前79.6 mm杆长和4 mm场半径，理论通带为
99.328～103.412 Th，N=256扫描的半高通带为99.5～103.0 Th，证明不修改几何即可形成质量选择；它
不包含边缘场，不能替代COMSOL/SIMION Candidate资格或机械Formal设计。
...
- `mass_filter_reference`通过L0理论/电压语义及L1有限长度质量扫描；仍未运行真实求解器质量扫描。
...
4. 碰撞冷却仍是独立后续功能；质量过滤下一步只需在同一几何上增加真实求解器RF+DC扫描，不能复用
   传输PASS，也不新建几何项目。
