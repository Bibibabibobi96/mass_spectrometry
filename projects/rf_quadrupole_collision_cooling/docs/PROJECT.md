...
1. 固定现有100 amu/N=100粒子表、RF工况和接口门禁，不调参追平结果。
2. 在入口、杆内和出口边缘的共同物理平面比较同粒子状态，定位束斑与发散差异首次稳定出现的位置。
3. 对COMSOL和SIMION分别做空间离散收敛，判断N=100接口FAIL来自边缘场离散、积分事件还是稳定的
   求解器差异；保留独立场，禁止把一端场导入另一端。
4. 只有N=100接口相空间达到预设目标或形成有误差预算支持的替代判据后，才生成面向OA-TOF集成的
   自包含候选部件包；当前不连接OA-TOF，也不进入碰撞或质量过滤阶段。
