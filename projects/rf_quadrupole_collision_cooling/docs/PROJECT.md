...
| S1 共面开孔静态参考 | oa屏蔽参数化开孔；RF出口与入口理想共面；无独立连接器、间隙、管或透镜 | 孔径可行区、静态联合场及N=100功能门禁 | 最简单物理拓扑参考，不证明脉冲或公差 |
| S2 被动接地连接器 | 只增加真实有限间距和接地管/外壳 | 三维边缘场、泄漏、损失和两端离散收敛 | 被动静态输运 |
...
这一事实。下一阶段S1仍被孔径可行区、参数化开孔、公共电位参考和联合场所有权阻塞。
...
必须同时吻合。未来候选只有在`束流下限 ≤ 设计完整高度 < 安全系数×min(全部理论上限)`时才通过，
等于上限、缺任一必需约束或手工重复常数均失败关闭，禁止运行打包。
