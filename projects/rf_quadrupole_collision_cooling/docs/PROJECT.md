...
资格仍为BLOCKED，且没有修改oaTOF Formal MPH、SIMION包或SolidWorks装配。旧
[`../config/rf_to_oatof_handoff.json`](../config/rf_to_oatof_handoff.json)和
[`../config/rf_to_oatof_interface_candidate.json`](../config/rf_to_oatof_interface_candidate.json)只保留为
投影诊断、S0/S1静态参考和历史run复现输入；不得作为与S2/S3并列的活动生产入口。

5 eV局部COMSOL源run仍可复核，但其旧下游`20260722_213000__sim__cross__rf-oatof-s1-physical-5ev__n100`
冻结的来源manifest身份已经漂移，故当前权威功能漏斗只引用上述可复核的S2/S3 gap0/gap1运行，不用该
旧下游run提升任何资格。
