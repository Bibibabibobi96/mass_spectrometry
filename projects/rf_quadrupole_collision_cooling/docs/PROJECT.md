...
这一事实。下一阶段S1仍被最低几何通过率、参数化开孔和联合场实现阻塞。
...
进一步降低接受率；当前尚未批准最低通过率、安全系数或最终孔径，S1状态仍为BLOCKED。
...
必须同时吻合。未来候选必须先满足`0 < 设计完整高度 < 安全系数×min(全部理论上限)`，再对每一份冻结
源束计算实际入口损失并达到另行批准的最低几何通过率。完整束流支撑不是默认硬下限；只有被批准损失
预算选中的分位包络才形成性能下限。等于上限、缺任一必需约束、未报告损失或手工重复常数均失败关闭，
禁止运行打包。
