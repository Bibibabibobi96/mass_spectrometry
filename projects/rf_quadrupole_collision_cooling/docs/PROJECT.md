...
不在项目内维护第二份档位定义。活动官方源迁移为`official_fixed_100.ion`；下文N=25运行均为迁移前
历史证据，不再授权当前Candidate或新功能闭合。RF-only、质量过滤和两类轴向加速等待新标准复验。

同一参考四极杆几何承载RF-only传输与RF+DC质量过滤两个已区分的功能模式。下述RF-only及有限几何质量扫描
都是迁移前N=25历史证据；求解器无关的L1扫描另行保留其已冻结的理论输入与边界。L1使用当前79.6 mm杆长和
...
迁移前`transport_no_collision + official_fixed_25.ion`曾在COMSOL 6.4和SIMION 2020完成独立场、粒子推进、
统一事件表、manifest和GUI资产复验，证明当前参考四极杆的RF-only径向约束与轴向传输。它不证明碰撞
冷却、质量过滤、机械正式几何或整机连接。
迁移前历史SIMION回归`20260722_233000__sim__simion__rf-transport__shared-runtime-n25__r02`保持25/25，并已改为
实际调用公共杆阵列、轴向接口布局和`common/multipole/simion_transport.lua`；正式COMSOL模型也消费
同一份派生杆坐标与极性组。迁移前历史COMSOL统一运行合同回归
...
迁移前历史四极杆运行用公共4段、0.4 mm绝缘间隙的杆内公共模阶梯实现轴向加速：保持RF开启，杆段
公共模由0降至-3 V，出口罩与检测参考区为-3 V。COMSOL N=25运行
`20260723_071800__sim__comsol__rf-quadrupole-axial-acceleration__n25__r01`为25/25，平均末端能量
5.0316 eV；相同分段几何和RF、零轴向压降对照为1.9949 eV，实测增益3.0367 eV。保存MPH已完成
GUI Compute及canonical粒子状态复验；该结论只是迁移前历史功能证据，不是当前闭合、分段优化、
收敛、SIMION或机械资格。
...
|---|---|---|---|
| 迁移前N=25历史回归（已失效） | 25/25，平均检测时间50.1155 µs | 25/25，平均检测时间49.7386 µs | 仅历史证据 |
...
迁移前历史SIMION功能run为`20260722_231100__sim__simion__mass-filter__shared-geometry-n175__r01`。同一份N=25源相空间只改变质量，
...
迁移前历史COMSOL功能run为`20260723_002556__sim__comsol__mass-filter__rf-dc-n175`。它在同一解析几何上分别
...
5. 轴向加速COMSOL功能已闭合；后续先补SIMION独立适配，再另行批准恒定2 eV、恒定5 eV和杆内
   `2→5 eV`对oaTOF接口的功能比较。仍禁止在handoff处重写速度伪造加速。
6. 分段数量、绝缘间隙、馈电、屏蔽连续性、局部网格和机械实现尚未优化，不得把当前4段参考当作
   正式硬件选择。
