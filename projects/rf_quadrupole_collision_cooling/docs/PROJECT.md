...
| 入口/出口边缘场 | 入口/杆区/出口相对场差为 3.46%/0.266%/3.22%；最大轴向切片差在 `z=1.8 mm` | 主要证据支持 |
...
| 推进器算法差异 | 两端各自时间加密均稳定；剩余项主要是 PA 场插值与 FEM 场表示对边缘差的累积响应 | 次要残余 |

横向差从入口边缘开始：均值在 `z=5.0 mm` 首次超过 0.01 mm，杆入口已为 0.0305 mm，随后在
杆内随 RF 相位漂移累积，并在出口边缘进一步放大。当前最强证据指向官方 0.2 mm PA 的离散边缘场与
连续 FEM 边缘场约 3% 的差异；杆中部聚焦梯度本身已闭合。该定位不要求、也不允许共享求解器场。
