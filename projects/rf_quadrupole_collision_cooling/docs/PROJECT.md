...
这一事实。下一阶段S1仍被参数化开孔构建和联合场实现阻塞；最低通过率在特征化阶段保持未定义，只有
...
上限`7.2 mm`，不是最终孔宽；最终横向值还必须与加速器轴心场比较，由离轴场均匀性、分辨率影响、
泄漏、机械和高压约束共同取交集。矩形孔中心仍为oa一级加速间隙中点，轴向至上下电极各保留
`1.05 mm`；横向值未解析前禁止生成候选几何，当前Formal屏蔽仍封闭，S1状态仍为BLOCKED。
...
`±0.5 mm`范围内的最坏轴向场轮廓RMS、势轮廓RMS和横向场RMS作为三个AND阈值；闭合屏蔽参考的连续
通过区为完整横向宽度`1.0 mm`，而`±0.75 mm`已失败。该结果只冻结比较判据和联合开孔扫描的L0起点；
真实侧孔会改变边缘场，必须在同一RF/oa联合开孔几何中重新采样后才能决定最终横向孔宽。
...
注入管场泄漏、三维场均匀性、高压/击穿和机械间距等上限，并乘显式安全系数，同时大于传播到入口的
...
的最大值；场均匀性等同时依赖横向宽度和轴向高度的约束必须保留为二维可行域，不能擅自拆成两个独立
