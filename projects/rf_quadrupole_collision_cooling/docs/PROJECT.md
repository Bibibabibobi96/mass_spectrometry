...
- `no_acceleration_full_length`已在v2合同下完成N=100功能数值资格矩阵：COMSOL空间/时间收敛、
  SIMION空间/时间收敛及各自收敛解的跨求解器功能闭合全部PASS。

这项新证据只授予无碰撞RF传输分类资格，不授予连续相空间等价、机械、Candidate或Formal资格；
其余上述历史数值仍只描述迁移前行为。
...
| 无碰撞RF传输功能 | v2 N=100五项数值矩阵全部PASS | 功能闭合；连续相空间/Candidate未授予 |
...
2. 为RF-only、RF+DC及轴向加速分别建立不互相替代的Candidate证据包；不得以功能run晋升。
3. 建立机械baseline、端部/屏蔽/馈通参数、CAD装配与GUI/CAD同步，再开放Formal门禁。
4. 若恢复RF→oaTOF接口资格工作，先单独批准目标与指标，再完成连接场数值资格、N=1000、
