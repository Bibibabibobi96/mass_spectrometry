...
`transport_no_collision_key_plane_distributions.png`。三图分别显示 25 条 `r(z)` 包络、同 ID 的
`|r_COMSOL-r_SIMION|`（均值/P95）与入口 0.2 mm、杆中点 45.6 mm、杆出口 85.4 mm、检测器前
94.8 mm 的并列束斑。当前最大均值/P95 径向差为 0.4457/0.9643 mm；杆内各离子仍远小于 `r0`，
因此这些图用于定位局部求解器差异，不替代传输率、TOF 和约束门禁。
