...
RF+DC质量过滤已完成求解器无关有限长度L1和真实SIMION有限几何功能扫描。L1使用当前79.6 mm杆长和
4 mm场半径，理论通带为99.328～103.412 Th，N=256扫描的半高通带为99.5～103.0 Th；SIMION按七个
质量、每质量25个配对粒子得到明确通带和端点抑制。两者共同证明不修改几何即可形成质量选择，但仍
不能替代独立COMSOL比较、分辨能力资格或机械Formal设计。
...
冷却、质量过滤、机械正式几何或整机连接。
SIMION回归`20260722_223100__sim__simion__rf-transport__shared-core-n25__r02`继续保持25/25，并已改为
实际调用`common/multipole/simion_transport.lua`；四极杆项目只保留单体几何、质量过滤和oaTOF接口适配。
...
- `mass_filter_reference`通过L0理论/电压语义、L1有限长度扫描和SIMION有限几何功能扫描；尚未完成
  独立COMSOL比较或质量分辨能力资格。
...
权威SIMION功能run为`20260722_210300__sim__simion__mass-filter__n175__r03`。同一份N=25源相空间只改变质量，
...
4. 碰撞冷却仍是独立后续功能；若继续质量过滤，下一步是在同一几何上建立独立COMSOL RF+DC响应并
   与SIMION比较，不能复用传输PASS，也不新建几何项目。
