...
2. 接口差异评估FAIL：N=100接口的束斑、发散和能量仍超出既定目标。局部场RMS加密变化与相空间
   ...
   区间和传播规则的完整误差预算。
3. 集成门禁FAIL：原N25/N100部件回归仍PASS，但严格接口FAIL，RF→oa-TOF变换/时间/接受度合同也
   尚未冻结；“下游功能接受度”替代判据为NOT_EVALUATED。因此不生成、不连接候选包。
...
求解器无关状态与派生格式可复现；oa-TOF外部handoff运行入口、电气接口和下游功能接受度仍未完成，
因此现有集成门禁结论继续为FAIL。该状态包需先由oa-TOF实际消费验证，再判断是否提升到`common/`。
