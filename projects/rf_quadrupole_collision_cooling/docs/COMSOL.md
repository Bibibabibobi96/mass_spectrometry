...
`electrode_or_boundary`终止粒子从标准脉冲前离子分布中排除，只在metadata记录损失数；不得仅凭有限
坐标称其“脉冲时存活”。图的时刻语义固定为脉冲开启前的左极限`t_pulse^-`。有限电场阶跃不使位置或
速度瞬时跳变，因此可使用开关边界的连续状态；图上不得混入此前已经终止但被数据集保持的坐标。
