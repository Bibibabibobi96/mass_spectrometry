...
`tests/comsol/build_s2_passive_connector_model.m`是S2几何的唯一MATLAB构建函数；build-only检查与场入口
共同调用它，禁止分别维护两套杆、屏蔽、1 mm连接器和oa加速器几何。oa baseline与加速器builder由
`config/rf_to_oatof_s2_dependencies.json`声明并在每个run中冻结、核对SHA-256；RF几何、位姿、孔径、
边界探针内移量、RF单位电压模式和网格参数分别从既有resolved/S1/S2合同读取，不在MATLAB中另存物理值。

`tests/comsol/run_s2_passive_connector_field.ps1`默认只求oa静态场和RF 100 V单位场两个Stationary基；
`-Particles`显式模式才创建CPT，并从manifest覆盖的真实RF出口事件通过统一handoff转换器派生canonical
输入。两种模式都不加入oa提取脉冲、不保存MPH。运行
`20260722_142733__analysis__comsol__rf-oatof-s2-no-pulse-field__gap1__r03`得到21个几何域和200712个总网格
单元；5个合同探针均为有限值，RF杆区1 mm离轴探针的单位场模为`12534.6156 V/m`。这只闭合名义场域
功能。当前统一入口的N=100默认运行`20260722_165527__sim__comsol__rf-oatof-s2-connector-gap1__n100`得到61个孔内oa
入口过面和39个端壁损失；事件必须同时满足首次正向到达入口平面和坐标位于参数化矩形孔内，不能把
实心端壁接触计作穿孔。`analysis/audit_s2_particle_chain.py`进一步验证ID、frame、clock epoch、三套年龄、
质量/电荷、过面坐标和速度-能量一致性。当前未做网格收敛、泄漏阈值、最低传输率、S2 PASS或Formal
资格验证。

0 mm直接共面案例由`config/rf_to_oatof_connector_cases.json`选择，并由解析器从同一S2基础合同派生，
不另存第二套几何。运行`20260722_164341__sim__comsol__rf-oatof-s2-connector-gap0__n100`得到20个几何域、
129589个网格单元、77个孔内入口事件和23个入口壁损失。直接共面时先在权威物理接口面按参数化孔径
分类，孔外粒子不送入CPT；孔内粒子才从下游真空侧做数值重启。物理接口状态、通过率和损失归因均不
使用数值偏移坐标。

当前数值重启仍使用合同给出的`0.001 mm`距离，并按法向速度同步推进启动时刻、按三维速度推进位置，
以避免把空间偏移误当成同一时刻的物理状态。它是临时求解器适配量，不是器件间距、孔厚或物理容差。
未来应验证COMSOL原生接口面/内部边界初始化方案并彻底删除该偏移；在此之前不据此作接口数值资格声明。

## S3脉冲前checkpoint派生诊断

`tests/analysis/run_rf_oatof_checkpoint_diagnostic.ps1`不调用COMSOL；它只接受显式成功的
`rf_to_oatof_s3_shared_clock_pulse_capture_n100`来源manifest，并在复制前后复验全部来源哈希。入口冻结
来源run的`particle_source`、`pulse_schedule`、S2 registration、S1合同、oa baseline以及manifest覆盖的
`s3_pulse_left_limit_state.csv`和全粒子终态账本，再调用求解器无关分析。尤其S2合同取自来源run的
`inputs.s2_contract`，不能用当前项目配置覆盖历史运行位姿。由此派生的metrics、逐粒子表和图不成为
第二套坐标、时钟、几何或物理参数源，也不回写COMSOL run。

N=100 gap1/gap0诊断分别得到31/39个脉冲前活动粒子；同ID真实捕获减无场弹道预测的`Δv_z` RMS分别为
188.141225/301.439136 m/s。该残差证明真实局部求解状态包含纯弹道模型没有的脉冲前场作用，但未隔离
RF边缘场、连接器间距、网格或数值重启的单独贡献。两案粒子幸存集合和脉冲时刻不同，不能用残差或
活动数直接选择间距。该分析没有运行新COMSOL求解、网格收敛、场扫描、时序扫描或N=1000统计，
`stage_passed`和`formal_gate_passed`均为false。
...
canonical入口状态必须位于S1合同的oa物理外表面`x=-67.8 mm`并使用`oatof_global`；它不保存为方便
求解而内移后的坐标。COMSOL仅在创建`ReleaseFromDataFile`时把该x加上合同中的`0.001 mm`真空侧数值
偏移，并在求解器启动前逐行验证x和frame。定时器读取同一物理面后再派生相同偏移。旧功能run输入表
曾写成`x=-62.8 mm`，但MATLAB实际释放和定时器都已独立使用`-67.799 mm`，所以历史物理轨迹不失效；
当前实现禁止这种静默覆盖再次发生。
