...
| COMSOL数值合同 | [`config/comsol_solver_numerics.json`](config/comsol_solver_numerics.json) |
...
多极杆分段杆和出口带孔接口板加速使用公共入口；具体命令与共同状态见公共multipole文档。RF→oaTOF物理连接
属于本项目专属集成链，当前累积入口为`tests/cross_solver/run_s3_cumulative_chain.ps1`。
...
`artifacts/projects/rf_quadrupole_collision_cooling/`，不进入Git源码树。

## 项目特有硬规则

- 接口输运、无碰撞回归、质量过滤和轴向加速是不同科学声明，不能由`Mode`或粒子数隐式切换。
- 两个求解器必须消费同一受治理粒子bundle及各自实际表示；未消费文件不能冒充来源证据。
- 质量过滤专属Mathieu判据、方形出口罩和RF→oaTOF连接不得上移到公共multipole层。
