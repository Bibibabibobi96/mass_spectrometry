...
for i=1:size(ions,1)
    ...
    if releaseGateEnabled
        ...
        releaseFile=readmatrix(releasePath,'FileType','text','Delimiter','tab');
        assert(isequal(size(releaseFile),[1 6]) && ...
            all(isfinite(releaseFile),'all') && ...
            max(abs(releaseFile-releaseData),[],'all')<1e-12, ...
            'Release file %d does not preserve its six-column source state.',i);
...
for index=1:expectedParticles
    ...
    assert(strcmp(char(feature.getString('icolp')),'0') && ...
        ...
        expectedTags{index});
    actualReleaseData=readmatrix(expectedPath,'FileType','text','Delimiter','tab');
    expectedReleaseData=[initialPositionMm(index,:) initialVelocityMS(index,:)];
    assert(isequal(size(actualReleaseData),[1 6]) && ...
        all(isfinite(actualReleaseData),'all') && ...
        max(abs(actualReleaseData-expectedReleaseData),[],'all')<1e-12, ...
        'Release file %03d differs from its frozen source-derived state.',index);
