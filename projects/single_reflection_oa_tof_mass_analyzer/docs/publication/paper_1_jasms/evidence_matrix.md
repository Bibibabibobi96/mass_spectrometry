...
| RF→OA integration历史observed authority | S1有850行共同脉冲时刻的连续单飞pre-pulse开发cohort和有序ID基础 | [`C1 source contract`](stage_c1_source_contract.md)判定S2仍无共同OA pre-pulse cohort；S1仅`DEVELOPMENT_ONLY`，须重建S2并锁定split |
