...
| S1 connector-gap pre-pulse residual | `C1_CONNECTOR_GAP_TRIPLET`的新 N=5000、real-field pulse-equivalent source-only stage：0/51.2/102.4 mm的可观测数为4558/393/117；配对轴向条件残差RMS为49.61/17.48/11.47 m/s，且每臂完整母cohort分母保留 | 这是残差—传输权衡的探测器盲证据，不是J2、峰宽或分辨率结论。51.2 mm须独立重建C1 source receipt后作为首个J2主工况；102.4 mm仅为低传输边界对照，当前样本不足以单独承担锁定J2结论 |
...
| S1三间隙真实场工作点机制实验 | [`20260827 connector-gap reports`](../../../../../../artifacts/projects/single_reflection_oa_tof_mass_analyzer/paper1_stage_evidence/connector_gap_working_point/20260827_112500/)绑定同一N=5000母cohort、每间隙冻结的pulse epoch和pre-pulse state；每对仅改变既有`z-vz`关系导出的加速器/反射器工作点。各臂用自己的完整检测命中峰形，且同一gap两臂的命中ID集合相同：0 mm为3679/5000、51.2 mm为157/5000、102.4 mm为59/5000。R分别为17842→18433（+3.3%）、11057→29989（+171%）、14122→47405（+236%）；1000次成对bootstrap的绝对R差异95%区间为0.42–5.02%、54.88–68.16%、56.99–77.88%。 | `MECHANISM_EXPERIMENT_PASS / MODEL_SPECIFIC`：与C1中条件残差49.61→17.48→11.47 m/s及母cohort传输下降一致，支持“低残差后调压才产生大聚焦收益”的受限数值机制；51.2 mm是主证据，102.4 mm仅为低传输边界。仍是单一终端八极杆、100 Th、冻结无碰撞独立粒子模拟；不能升级为普遍真实仪器性能或`first/novel`主张。 |
