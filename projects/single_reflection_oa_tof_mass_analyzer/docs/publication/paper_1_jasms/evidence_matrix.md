...
| RF→OA integration历史observed authority | S1有850行共同脉冲时刻的连续单飞pre-pulse开发cohort和有序ID基础 | [`C1 source contract`](stage_c1_source_contract.md)判定S2仍无共同OA pre-pulse cohort；最新914-hand-off真实PA筛查表明当前侧入口沿`x`传播、而OA提取沿`z`，必须先冻结轴向注入或真实转向接口；S1仅`DEVELOPMENT_ONLY` |
