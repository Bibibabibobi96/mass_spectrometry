...
- 该映射现由`paper1_c3_j3_mapping.py`逐点重推：它要求C2的improve/zero/worsen是对称方向，并把zero点逐项绑定到冻结T5 Candidate。S1已通过该输入身份检查；仍须由integration把每一个派生Candidate编译为独立PA/IOB后才可启动N=1。
