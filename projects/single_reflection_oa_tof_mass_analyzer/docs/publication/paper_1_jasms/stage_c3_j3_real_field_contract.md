...
- 新的`CandidateControlRequest`把每个物理电压和grid2/exit几何扰动、绝对边界与`-2h,-h,0,+h,+2h`一次性冻结。它不读取探测器结果，也不选择“最好”的扰动。
- 请求必须显式给出从C2_J3抽象方向到物理电极方向的映射。该映射尚未冻结，故本合同不能启动SIMION。不得把理想`Γ3`系数直接当作电压。
