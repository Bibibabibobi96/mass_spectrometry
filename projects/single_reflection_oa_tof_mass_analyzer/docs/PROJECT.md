...
当前理想场数值源比较已贯通自动残差扫描、两区/三区束宽扫描和可校验断点续算。
在指定工作点下支持低残差调压收益显著、随残差增加收益衰减；最高已测残差仍有12%—36%收益，
不支持“调压完全无效”。R≥25000的连续已测接受宽度：残差0/1 m/s时两区至2.2 mm、三区至少
2.8 mm；残差10 m/s时分别至1.5/2.6 mm。结果是理想轴向、固定外部几何的局部比较，不是全局
最优或实际孔径传输；完整条件、负结果及证据见
[完成快照](history/20260827__ideal-source-residual-and-width-comparison.md)。

在独立的仿射斜率机制比较中，统一人造源的中心速度、1.0 mm全宽和`11.1251 m/s`随机残差保持不变；
只令`k=0`、历史`37.7918 m/s/mm`标量参考及当前`228.8060 m/s/mm`斜率改变。每个非零`k`都以
`a1=a2=a3=0`重算依赖的后两区场/长度和反射器二级场，而不是把电压作为独立峰宽调参。三组各5000粒子
的精确一维运行显示：`k=0`收益恒为零；历史参考斜率的匹配工作点相对零斜率工作点提高`42.6%–59.2%`；
当前人造源斜率提高`708.6%–776.4%`。这支持“当源的线性位置—速度关系已知时，三区理论工作点可以利用
它”的机制命题；它不是三维场、真实RF源、传输率、全局最优或投稿新颖性已关闭的证据。完整输入、工作点
及负对照见[斜率扫描快照](history/20260827__synthetic-affine-slope-scan.md)。
