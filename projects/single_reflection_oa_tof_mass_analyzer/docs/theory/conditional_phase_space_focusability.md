...
- [1. 切向closure与条件厚度](#1-切向closure与条件厚度)
- [2. 尺度固定与残差模态](#2-尺度固定与残差模态)
- [3. 保持既有约束后的可行控制方向](#3-保持既有约束后的可行控制方向)
- [4. Source-whitened控制子空间](#4-source-whitened控制子空间)
- [5. 沿完整条件分布的离散实现](#5-沿完整条件分布的离散实现)
- [6. Focusability index](#6-focusability-index)
- [7. 电压、几何和信赖域约束](#7-电压几何和信赖域约束)
- [8. 新增场区或控制方向的增量价值](#8-新增场区或控制方向的增量价值)
- [9. `Γ3`的正确定位](#9-γ3的正确定位)
- [10. SVD、rank与可调性](#10-svdrank与可调性)
- [11. 有限孔径和命中拓扑边界](#11-有限孔径和命中拓扑边界)
- [12. 二阶、非高斯和多模态限制](#12-二阶非高斯和多模态限制)
- [13. 可证伪假设](#13-可证伪假设)
- [14. 最低验证要求](#14-最低验证要求)
- [15. 设计决策图](#15-设计决策图)
- [16. 当前证据边界](#16-当前证据边界)
...
对最终可观测的全源方差，必须同时保留全方差公式：

$$
\operatorname{Var}(T)=E_{\mathbf s}[\operatorname{Var}(T\mid\mathbf s)]
+\operatorname{Var}_{\mathbf s}[E(T\mid\mathbf s)].
$$

因此局部设计不能只最小化`J_\perp`。在离散source bins `j`上，本文采用的联合线性目标是

$$
\boxed{J_{\mathrm{tot,lin}}(\eta)=
\operatorname{Var}_j[\tau_{j0}+h_j^T\eta]
+\sum_jw_j\|L_j^T(g_j+G_{N,j}\eta)\|_2^2}.
$$

第一项防止为了减小条件厚度而放大条件均值之间的到达时间离散；第二项才是本文的source-whitened
focusability项。两项均为局部近似，最终必须由完整母cohort直接峰形和传输率检验。
...
`J_{\perp,\mathrm{lin}}^{\min}`不是非线性直接峰宽、总方差、全局架构最优或物理分辨率下限；它只能作为
冻结工作点、固定事件拓扑和无界局部线性模型的参考值。
...
信赖域必须由直接粒子步长平台预先识别：在该域内，有限差分导数、两条独立导数实现和直接方差变化应
一致；一旦发生孔径、栅线、折返或检测器事件序列变化，当前局部模型立即失效并必须重建，而不能把
跨拓扑差异解释为连续导数收益。
...
本文的source是冻结的、无碰撞且独立粒子的RF多极杆模拟状态。它不代表实测源，也不包含空间电荷、
碰撞冷却或集体效应。报告发射度或相空间压缩时必须区分保守重匹配、碰撞冷却、孔径裁剪和仅存活粒子
造成的表观变窄；不得以Liouville定理为由忽略损失或把survivor-only统计称为源本身变冷。
...
本文保持 `PAPER_1_CANDIDATE / EVIDENCE_REQUIRED`。当前已实现部分、历史开发证据、前瞻源要求和
独立验证缺口统一见[Paper 1 证据矩阵](../publication/paper_1_jasms/evidence_matrix.md)。
本文推导不能改变项目 Formal 资格，也不能单独支持投稿结论。
