...
`z_vz_linear_phase_space_coupling.md`；本文以下静止释放推导不重复扩写该模型。
...
本文不保存当前项目电压、尺寸、候选状态或正式结果。当前事实仍以项目 `config/` 和 `docs/PROJECT.md` 为准。本文件替代已归档的[`三栅加速器总长度符号推导.docx`](../history/20260721__superseded-theory-docx/三栅加速器总长度符号推导.docx)；旧 DOCX 只作为带日期和 superseded 标记的历史证据，不能继续作为当前公式权威。
