...
for line in input:lines() do
  ...
  if time_us <= 2 then
    ...
    local inside_pa = xg >= 0 and xg <= instance.pa.nx-1
      and yg >= 0 and yg <= instance.pa.ny-1
      and zg >= 0 and zg <= instance.pa.nz-1
    if inside_pa then
...
input:close()
output:close()
assert(count > 0, 'no trajectory samples lie inside the accelerator PA')
report:write(string.format('EXPORTED_ROWS=%d\nSKIPPED_OUTSIDE_PA=%d\nSTATUS=PASS\n',count,skipped))
report:close()
