...
$candidateMph = Join-Path $candidateRoot 'comsol\single_reflection_oa_tof_mass_analyzer__candidate.mph'
...
[ordered]@{
  schema_version=1; run_id=$RunId; project='single_reflection_oa_tof_mass_analyzer'; mode='coupled_baseline_validation'
...
function Invoke-PairedComparison([string]$Left, [string]$Right, [string]$Output, [string]$LeftLabel, [string]$RightLabel) {
  & $python -m projects.single_reflection_oa_tof_mass_analyzer.analysis.reference_analysis compare $Left $Right --mass 524 --output $Output `
