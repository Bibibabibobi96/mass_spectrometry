...
$acceleratorComsolSource = Join-Path $repoRoot 'projects\orthogonal_accelerator\comsol\build_two_zone_geometry.m'
$acceleratorGridComsolSource = Join-Path $repoRoot 'projects\orthogonal_accelerator\comsol\build_two_zone_grids.m'
$comsolSources += @($acceleratorComsolSource, $acceleratorGridComsolSource)
...
$simionDeliveryBuilder = Join-Path $componentDir 'simion\workbench\build_formal_delivery.ps1'
...
$acceleratorGemPath = Join-Path $repoRoot 'projects\orthogonal_accelerator\simion\two_zone_accelerator.gem'
...
$syncOutput = & $python -m projects.single_reflection_oa_tof_mass_analyzer.analysis.sync_geometry_contract --check
...
if ($contract.simion_runtime.ideal_grid_model -ne 'simion_one_row_zero_width_native_transmission') {
  throw 'SIMION ideal-grid model must use official one-row zero-width native transmission'
}
Assert-NotContains $lua 'ideal_grid_epsilon_mm' 'SIMION retired generic grid teleport epsilon'
Assert-NotContains $lua 'accelerator_grid_epsilon_mm' 'SIMION retired accelerator grid teleport epsilon'
Assert-NotContains $lua 'reflectron_grid_epsilon_mm' 'SIMION retired reflectron grid teleport epsilon'
Assert-NotContains $lua 'grid_planes' 'SIMION retired grid teleport plane table'
Assert-NotContains $lua 'inside_grid' 'SIMION retired grid teleport footprint gate'
Assert-NotContains $lua 'grid_jump' 'SIMION retired grid teleport counter'
Assert-NotContains $lua 'jumped' 'SIMION retired grid teleport state'
Assert-NotContains $lua 'ion_pz_mm=post' 'SIMION retired particle relocation'
Assert-NotContains $lua "{name='grid1'" 'SIMION accelerator grid1 teleport plane'
Assert-NotContains $lua "{name='grid2'" 'SIMION accelerator grid2 teleport plane'
Assert-NotContains $lua "{name='entgrid'" 'SIMION reflectron entrance-grid teleport plane'
Assert-NotContains $lua "{name='midgrid'" 'SIMION reflectron mid-grid teleport plane'
...
Assert-Contains $acceleratorGem "# local interface_port_enable = assert(var.interface_port_enable, 'interface_port_enable is required')" 'SIMION explicit interface-port topology'
Assert-Contains $acceleratorGem '# if interface_port_enable ~= 0 then' 'SIMION candidate interface-port switch'
Assert-Contains $acceleratorGem '$(interface_port_width_y),$(interface_port_height_z)' 'SIMION candidate interface-port dimensions'
...
Assert-Contains $acceleratorGem '$(mmgu_xy),$(mmgu_xy),$(mmgu_z), surface=none)' 'SIMION accelerator official ideal-grid PA surface mode'
Assert-NotContains $acceleratorGem 'surface=fractional' 'SIMION accelerator retired fractional surface mode'
Assert-Contains $acceleratorGem '$(electrode_width/2),$(electrode_width/2),$(stage1_length))' 'SIMION accelerator grid1 equal-endpoint zero-width sheet'
Assert-Contains $acceleratorGem '$(shield_inner_width/2),$(shield_inner_width/2),$(stage1_length+stage2_length))' 'SIMION accelerator grid2 equal-endpoint zero-width sheet'
Assert-Contains $acceleratorGem 'e($(3+ring_count)) { fill { within { box3D(' 'SIMION parameterized accelerator exit grid entity'
Assert-Contains $acceleratorGem 'e($(4+ring_count)) { fill {' 'SIMION parameterized accelerator shield electrode'
Assert-Contains $reflectronGem '$(mmgu_axial),$(mmgu_radial),$(mmgu_radial),surface=none)' 'SIMION reflectron official ideal-grid PA surface mode'
Assert-NotContains $reflectronGem 'surface=fractional' 'SIMION reflectron retired fractional surface mode'
Assert-Contains $comsol "interfacePort = struct('enabled', false)" 'COMSOL formal-default closed interface port'
Assert-Contains $comsol "add_block(geom1,'accelshieldPort'" 'COMSOL candidate interface-port cut'
Assert-Contains $comsol "cuts{end+1} = 'accelshieldPort'" 'COMSOL candidate port subtraction'
Assert-Contains $comsol "'grid1_z', 'z_accel_grid1', 'stage2_length', 'accel_stage2_length'" 'COMSOL accelerator stage parameters'
...
Assert-Contains $flightTubeBuilder "local wall = assert(tonumber(arg[7]), 'invalid wall thickness')" 'SIMION field-free builder required wall'
...
Assert-Contains $reflectronBuilder "local wall = assert(tonumber(arg[7]), 'invalid wall thickness')" 'SIMION reflectron builder required wall'
Assert-Contains $reflectronBuilder "local far_clearance = assert(tonumber(arg[10]), 'invalid far clearance')" 'SIMION reflectron builder required clearance'
...
Assert-Contains $deliveryBuilder '[string]$ContractPath' 'SIMION candidate resolved-contract option'
...
Assert-Contains $comsol 'build_two_zone_grids(geom1,struct(' 'COMSOL accelerator-grid component delegation'
Assert-Contains $comsol "'grid2_z','z_accel_grid2'" 'COMSOL accelerator exit-grid coordinate contract'
Assert-Contains $comsol "'grid2_width','2*accel_shield_half'" 'COMSOL accelerator exit-grid aperture contract'
...
if (-not $SkipRuntime) {
  ...
  if ($formalRelease.project -ne 'single_reflection_oa_tof_mass_analyzer' -or $formalRelease.role -ne 'formal_asset_manifest') {
  ...
  if ($formalRelease.validation_contract.path -ne 'projects/single_reflection_oa_tof_mass_analyzer/config/formal_validation.json' -or
