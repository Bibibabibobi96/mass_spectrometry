...
& $python -m projects.single_reflection_oa_tof_mass_analyzer.analysis.resolve_geometry --check
...
if ($Level -eq 'Candidate') {
  ...
  elseif ($CandidateTarget -eq 'COMSOL') {
    ...
    $runDir = Join-Path $workspaceRoot "artifacts\projects\single_reflection_oa_tof_mass_analyzer\runs\$runId"
    ...
    [ordered]@{schema_version=1;run_id=$runId;project='single_reflection_oa_tof_mass_analyzer';mode='comsol_candidate_sync_gate';project_root=$projectRoot;inputs=[ordered]@{candidate_model=$candidateModel};formal_gate_passed=$false} |
...
elseif ($Level -eq 'Formal') {
  ...
  $formalModel = Join-Path $workspaceRoot 'artifacts\projects\single_reflection_oa_tof_mass_analyzer\formal\comsol\single_reflection_oa_tof_mass_analyzer__model.mph'
  $formalStamp = Get-Date -Format 'yyyyMMdd_HHmmss'
  $formalRunId = "${formalStamp}__gate__comsol__formal-sync"
  $formalRunDir = Join-Path $workspaceRoot "artifacts\projects\single_reflection_oa_tof_mass_analyzer\runs\$formalRunId"
  ...
  [ordered]@{schema_version=1;run_id=$formalRunId;project='single_reflection_oa_tof_mass_analyzer';mode='comsol_formal_sync_gate';project_root=$projectRoot;inputs=[ordered]@{formal_model=$formalModel};formal_gate_passed=$true} |
