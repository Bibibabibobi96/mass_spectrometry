function result = oatof_build_model_core(mass_amu, label, solver_mode, field_mode, d1_mm, n_rings2, mesh_hmax_refl_mm, bore_r_mm, ring_thickness_mm, n_particles, n_rings1, accel_bore_half_mm, fixed_particle_table, fine_tstep_ns, mesh_hmax_accel_mm, drift_tstep_ns, output_model_path, contract_path, allow_particle_loss)
...
if nargin < 19 || isempty(allow_particle_loss)
    allow_particle_loss = false;
end
...
accelringtags = oatof_build_accelerator_geometry( ...
    geom1,ringDesign.accelerator_count);
...
% The resolved accelerator ring count controls the same closed-form
% geometry and voltage spacing.
for k = 1:ringDesign.accelerator_count
    tagk = sprintf('accelring_%d', k);
    tagb = sprintf('selb_%s', tagk);
    potk = es.create(sprintf('pot_%s', tagk), 'ElectricPotential', 2);
    potk.label(sprintf('Accelerator ring %d graded potential', k));
    potk.selection.named(tagb);
    potk.set('V0', sprintf('V_grid1*(1-%d/%d)', ...
        k,ringDesign.accelerator_count+1));
end
for k = 1:n_rings1
...
[~,~,t_mesh] = oatof_build_mesh( ...
    model,comp1,p,mesh_hmax_refl_mm,mesh_hmax_accel_mm,contract.comsol_runtime);
...
nominal_energy_per_charge_v = ...
    contract.geometry_derivation.reflectron.nominal_energy_per_charge_V;
v_push_ffcheck = sqrt(2*nominal_energy_per_charge_v*1.602176e-19/ ...
    (mass_amu*1.66054e-27));
...
rel1 = particle.rel1; tstep = particle.tstep;
...
if n_detected_check < N_total_check
    fprintf('[%s] short margin insufficient -- re-solving with resolved fallback factor %.3g (Tsim=%.4gus).\n', ...
        label,contract.comsol_runtime.fallback_time_factor,Tsim_full*1e6);
    Tsim = Tsim_full;
...
% One extractor owns return-leg selection, crossing interpolation,
% Wall/Freeze handling and active-radius acceptance for every COMSOL path.
detector_z_exact = p.evaluate('detector_z','mm');
detector_x_exact = p.evaluate('detector_x','mm');
detector_radius = p.evaluate('detector_radius','mm');
arrivals = oatof_extract_detector_arrivals( ...
    t(:),x_full,y_full,z,detector_z_exact,1e-3,0.5, ...
...
completeEvent = isfinite(t_accel_gate) & isfinite(t_refl_entry_gate) & ...
    isfinite(t_refl_exit_gate) & arrivals.hit.';
if allow_particle_loss
    assert(nDet >= 2 && sum(completeEvent) == nDet, ...
        ['Every detector hit must have accelerator and reflectron events; ' ...
         'at least two detector hits are required.']);
else
    assert(all(completeEvent) && nDet == nP, ...
        'Event-window gate could not identify every accelerator/reflectron/detector event.');
end
t_accel_complete = t_accel_gate(completeEvent);
t_refl_entry_complete = t_refl_entry_gate(completeEvent);
t_refl_exit_complete = t_refl_exit_gate(completeEvent);
det_complete = detTimes(completeEvent);
assert(max(t_accel_complete) < timing.t_accel_end_s-0.1e-6, ...
    'Accelerator exit is too close to/outside the fine-window end.');
assert(min(t_refl_entry_complete) > timing.t_refl_start_s+0.25e-6, ...
    'Reflectron entry is too close to/outside the fine-window start.');
assert(max(t_refl_exit_complete) < timing.t_refl_end_s-0.25e-6, ...
    'Reflectron return exit is too close to/outside the fine-window end.');
assert(min(det_complete) > timing.t_detector_start_s+0.1e-6 && ...
    max(det_complete) < timing.t_detector_end_s-0.1e-6, ...
...
fprintf(['[%s] event-window gate PASS: accel exit <=%.4fus; reflectron ' ...
    'entry %.4f-%.4fus, exit %.4f-%.4fus; detector %.4f-%.4fus; complete=%d/%d.\n'], ...
    label, max(t_accel_complete)*1e6, min(t_refl_entry_complete)*1e6, ...
...
if nP > PLOT_RESOLVE_THRESHOLD
    ...
    t_plot = pd_plot.t;
    x_plot = squeeze(pd_plot.p(:,:,1)); z_plot = squeeze(pd_plot.p(:,:,3));
    ...
    nP_plot = min(50, nP);
    t_plot = t;
    x_plot = x_full(:,1:nP_plot); z_plot = z(:,1:nP_plot);
...
if isempty(output_model_path)
    error(['OutputModelPath is required for every run; ordinary model builds ' ...
        'cannot select or overwrite Formal assets.']);
end
modelPath = char(output_model_path);
modelsDir = fileparts(modelPath);
formalRoot = string(java.io.File(paths.comsolFormalDir).getCanonicalPath());
resolvedModelPath = string(java.io.File(modelPath).getCanonicalPath());
assert(~startsWith(lower(resolvedModelPath),lower(formalRoot+filesep)), ...
    'Ordinary COMSOL builds cannot write Formal; use the promotion transaction.');
