...
```powershell
.\.venv\Scripts\python.exe -m projects.single_reflection_oa_tof_mass_analyzer.analysis.reference_analysis single `
...
```powershell
.\.venv\Scripts\python.exe -m projects.single_reflection_oa_tof_mass_analyzer.analysis.reference_analysis simion-recording `
...
- `analyze_accelerator_transverse_field_uniformity.py`比较轴心与严格正负对称偏轴位置的加速场轮廓、
  ...
  它调用`../comsol/export_accelerator_transverse_field_uniformity.m`，不重求解或追踪粒子，结果不能
  替代开孔联合场和最终分辨率门禁。
