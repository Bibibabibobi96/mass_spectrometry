...
旧末端阶跃诊断曾在多极杆terminal/handoff强边缘场位置用动能判断10 eV，因此其
`9.70503/9.99052 eV`比较不再是目标注入能量证据。当前统一口径只在连续链的
`pre_pulse_state`验证：离子必须位于oaTOF repeller与grid1之间的加速器内部，且采样发生在脉冲前；
terminal/handoff能量只保留为接口诊断。入口参考套筒现为`1.5 mm`内径、下游端面与杆端`z=0`共面，
套筒与入口板均为`+8 V`。修正几何的N=100真实SIMION得到末端/分段方案检测传输`98/100`与`64/100`，
加速器内平均动能分别为`10.02390±0.05540 eV`（n=98）和
`10.02481±0.06896 eV`（n=64）；共同64粒子的配对均值差仅`-0.00093 eV`。这确认两方案达到同一
目标能量，当前差异是传输与相空间选择，而不是最终能量。完整修正记录见
...
后续2.2 mm oaTOF理论源宽候选保持同一真实八极杆输出，传输仍为948/1000，且没有改变脉冲前z展宽或
加速器出口角度；因此当前上游相空间结论不变。该候选的自动理论闭合、PA重建及完整数值只见
