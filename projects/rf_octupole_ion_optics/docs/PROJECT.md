...
同日的末端阶跃诊断把四段杆统一为`+8 V`并在`0 V` oaTOF入口完成唯一电势降。N=1000八极杆
handoff为893/1000，高于分段方案577/1000；连续链检测为948/1000，高于612/1000，但handoff
正交加速方向角度σ和脉冲前σz反而从`1.15464°/0.46245 mm`增至`1.75191°/0.51764 mm`。
两方案确实包含相同`+8 V`入口参考套筒；新增RF-off共同粒子ID 16对照得到末能量
`9.70503/9.99052 eV`，证明短开口套筒没有使两种全局电极拓扑下的源点实际静电势相同。因此当前
更高传输和更窄诊断峰不能表述为严格同10 eV的最终拓扑优势。完整运行、固定共同群和源区修复边界见
...
结论保持`INCONCLUSIVE_DIAGNOSTIC_ONLY`；下一步先闭合入口参考区等势性，不以减孔代替边界修复。
