...
- 先前受治理运行`20260724_135148__test__comsol__wehnelt-build-only-smoke`完成了真实三阶段构建，但冻结
  Static gate在`inputs/`内生成了15个未列入manifest的Python/Ruff缓存文件；其51项声明输入和商业构建
  输出哈希仍有效，目录全量封存语义则未闭合。随后运行
  `20260724_141458__test__comsol__wehnelt-build-only-smoke`在商业调用前发现首次冻结manifest transition
  产生的两个Python缓存并按`failed`闭合，53项实际输入全部进入失败manifest，商业wrapper未被调用。
  两个run均保留为现行provenance门禁的前序证据，不再是当前开放阻断，既有目录不得追写或清理。
- 当前闭合运行`20260724_143420__test__comsol__wehnelt-build-only-smoke`已在提交`a7e952f`的干净Wehnelt
  源码上，用51项冻结输入完成真实MATLAB R2025b/COMSOL 6.4 build-only三阶段构建。声明输入与`inputs/`
  实际文件一一相等，无重复、Python bytecode或Ruff缓存，且全部SHA-256匹配该提交，现行provenance缺口
  已关闭。几何、网格、静电模型树、CPT树、NIST电子质量/电荷和`Freeze`壁面条件绑定、GUI可见参数的
  MATLAB API绑定均通过，三个MPH非空；静电与粒子求解器均未运行。首次商业启动在120秒内未生成报告，
  公共wrapper按受治理策略完成一次clean retry后成功，因此success manifest共有8项输出：6项非空核心
  输出（三个MPH、严格报告、wrapper日志和summary）及2项显式记录且哈希闭合的首次启动诊断日志，其中
  空stderr为0字节。wrapper最终明确记录invocation attempted、completed、exit 0和returned。该证据只证明
  build-only模型树与合同绑定，不证明GUI Compute、求解器结果、性能、Candidate或Formal资格。
...
2. 后续功能运行必须继续形成完整run config、summary、manifest和资产SHA；最终CPT模型还必须重开并
   完成GUI Compute对等复算，在此之前不得提升当前资产资格。
