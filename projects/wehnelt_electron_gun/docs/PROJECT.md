...
- runner现已实际执行冻结副本中的resolver和Static gate，严格解析唯一`KEY=VALUE`报告并拒绝未知、重复、
  ...
  manifest写入失败留下伪终态。以上治理已通过非商业故障与冻结快照测试，但尚未用现行runner重新运行
  一次R2025b/COMSOL build-only smoke，因此不能把既有商业run当作这版runner治理的验证。
...
1. 用现行`build_only_smoke`重新做一次真实R2025b三阶段构建和GUI节点复验；低N fixture不产生
   Candidate证据。
2. 为`functional_reference`建立受治理的N=100运行器，先冻结可复现母样本、逐粒子身份、终态和完备
