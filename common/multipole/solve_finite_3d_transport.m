...
required = {reportPath, resolvedDesignPath, numericsPath, sourcePath, sourceMetadataPath, ...
    runtimeDir, solverProgressDir, eventsPath, trajectoryPath, modelPath};
...
try
    ...
    if segmentHybridMesh
        ...
        if isfield(hybrid, 'sensitive_region')
            ...
            if isfield(sensitive, 'exit_interface_refinement')
                exitRefinement = sensitive.exit_interface_refinement;
                assert(isfinite(exitRefinement.upstream_extent_from_rod_exit_mm) && ...
                    exitRefinement.upstream_extent_from_rod_exit_mm > 0 && ...
                    ...
                    ['Exit-interface refinement must not enter a swept rod interior; ' ...
                    'its upstream extent must be positive and no larger than the ' ...
                    'segment-end buffer, and its downstream extent must be nonnegative.']);
                exitRefinementZMin = d.rod_z_max - ...
                    exitRefinement.upstream_extent_from_rod_exit_mm;
                exitRefinementZMax = d.handoff_plane_z + ...
                    exitRefinement.downstream_extent_from_handoff_mm;
                assert(exitRefinementZMin >= d.vacuum_z_min && ...
                    exitRefinementZMax <= d.vacuum_z_max && ...
                    exitRefinementZMax > exitRefinementZMin, ...
                    'Exit-interface refinement must lie inside the resolved vacuum domain.');
                geom.feature.create('meshSensitiveExit', 'Cylinder');
                geom.feature('meshSensitiveExit').set('r', ...
                    sprintf('%.17g[mm]', sensitive.particle_corridor_radius_mm));
                geom.feature('meshSensitiveExit').set('h', ...
                    sprintf('%.17g[mm]', exitRefinementZMax-exitRefinementZMin));
                geom.feature('meshSensitiveExit').set('pos', ...
                    {'0','0',sprintf('%.17g[mm]', exitRefinementZMin)});
                geom.feature('meshSensitiveExit').set('selresult', 'on');
            end
        end
    ...
    if segmentHybridMesh
        ...
        if isfield(meshPrebuildDiagnostics, 'exit_interface_refinement')
            assert(meshPrebuildDiagnostics.exit_interface_refinement.domain_count > 0 && ...
                meshPrebuildDiagnostics.exit_interface_refinement.feature_present, ...
                'Exit-interface refinement sizing is incomplete.');
        end
    end
    mesh.run;
    ...
    [onEvents, onTrajectories] = analyze_particle_case( ...
    ...
    [zeroEvents, zeroTrajectories] = analyze_particle_case( ...
...
exitInterfaceRefinement = isfield( ...
    hybridSelections, 'tetrahedral_exit_refinement');
if localized
...
if exitInterfaceRefinement
    exitRefinementSizeCount = double(any(strcmp( ...
        tetrahedralFeatureTags, 'szTetExit')));
    exitRefinementExpectedSizeCount = double(~isempty(selection_entities( ...
        comp, hybridSelections.tetrahedral_exit_refinement)));
    exitRefinementDomains = selection_entities( ...
        comp, hybridSelections.tetrahedral_exit_refinement);
else
    exitRefinementSizeCount = 0;
    exitRefinementExpectedSizeCount = 0;
    exitRefinementDomains = [];
end
...
for index = 1:numel(hybridSelections.sweep)
    ...
    if exitInterfaceRefinement && ~isempty(selection_entities( ...
            comp, hybridSelections.sweep_exit_refinement{index}))
        exitRefinementExpectedSizeCount = exitRefinementExpectedSizeCount + 1;
        exitRefinementSizeCount = exitRefinementSizeCount + double(any(strcmp( ...
            sweepFeatureTags, sprintf('szExit%d', index))));
        exitRefinementDomains = union(exitRefinementDomains, selection_entities( ...
            comp, hybridSelections.sweep_exit_refinement{index}));
    end
end
...
if exitInterfaceRefinement
    diagnostics.exit_interface_refinement = struct( ...
        'domain_count', numel(exitRefinementDomains), ...
        'size_feature_count', exitRefinementSizeCount, ...
        'expected_size_feature_count', exitRefinementExpectedSizeCount, ...
        'feature_present', exitRefinementSizeCount == ...
            exitRefinementExpectedSizeCount);
end
...
if exitInterfaceRefinement
    fprintf(fid, 'MESH_EXIT_INTERFACE_REFINEMENT_PRESENT=1\n');
    fprintf(fid, 'MESH_EXIT_INTERFACE_REFINEMENT_DOMAIN_COUNT=%d\n', ...
        diagnostics.exit_interface_refinement.domain_count);
    fprintf(fid, 'MESH_EXIT_INTERFACE_REFINEMENT_SIZE_FEATURE_PRESENT=%d\n', ...
        diagnostics.exit_interface_refinement.feature_present);
    fprintf(fid, 'MESH_EXIT_INTERFACE_REFINEMENT_SIZE_FEATURE_COUNT=%d\n', ...
        diagnostics.exit_interface_refinement.size_feature_count);
    fprintf(fid, 'MESH_EXIT_INTERFACE_REFINEMENT_SIZE_FEATURE_EXPECTED_COUNT=%d\n', ...
        diagnostics.exit_interface_refinement.expected_size_feature_count);
end
...
function [events, trajectories] = analyze_particle_case(pd, source, caseId, ...
