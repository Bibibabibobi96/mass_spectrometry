...
if ([string]::IsNullOrWhiteSpace($RunId)) {
  $taskLabel = $projectId.Replace('_','-') + $(if ($AxialAcceleration) {'-axial-acceleration'} else {'-finite-3d-interfaces'})
...
[ordered]@{
  ...
  run_id = $RunId
  project = $projectId
  mode = $(if ($AxialAcceleration) {'axial_acceleration_reference'} else {'finite_3d_no_collision'})
    ...
    try {
      ...
      if ($AxialAcceleration) {
        & $python -m common.multipole.axial_acceleration --contract $axialAccelerationBase `
          --rod-geometry $roundRodGeometry --source-energy-ev 2.0 --charge-state 1 --output $axialAccelerationResolvedPath
