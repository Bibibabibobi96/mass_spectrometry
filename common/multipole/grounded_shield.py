...
def render_grounded_circular_to_rectangular_connection(
    *,
    ...
    if min(aperture_width_mm, aperture_height_mm, cell_mm) <= 0:
    ...
    if sleeve_length > 0:
        lines.extend(
            [
                ...
                f"    notin_inside {{ locate({fmt(sleeve_x_max_mm+cell_mm)},{fmt(center_y_mm)},{fmt(center_z_mm)},1,90) {{ cylinder(0,0,0,{fmt(inner_radius_mm)},,{fmt(sleeve_length+2*cell_mm)}) }} }}",
                "  } }",
            ]
        )
