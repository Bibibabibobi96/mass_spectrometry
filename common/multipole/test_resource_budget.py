...
class ResourceBudgetTests(unittest.TestCase):
    ...
    def test_only_quadrupole_no_acceleration_temporal_screen_is_authorized(self) -> None:
        ...
        self.assertEqual(
            resolved["limits"]["minimum_system_available_memory_bytes"], 8 * 1024**3
        )
        self.assertEqual(resolved["limits"]["compact_final_retained_bytes"], 25 * 1024**2)
        self.assertEqual(resolved["limits"]["automatic_retry_count"], 0)
        for project_id in ("rf_hexapole_ion_optics", "rf_octupole_ion_optics"):
