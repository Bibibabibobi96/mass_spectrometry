...
与PA refine、离子飞行及其他重任务串行；PA输入准备、GEM转PA和缓存物化默认轻任务。
轻任务按公共预算在CPU、内存和I/O准入仍满足时可与之并行，调用方不能根据瞬时低利用率自行改写任务类别。
`-ResourceBudgets` 可传入冻结资源
...
无须先完成专项峰值测量；未列入中央重任务名单的阶段默认轻任务。本入口的solver明确列为重任务，
且不开放多个受管COMSOL服务器会话并行。
