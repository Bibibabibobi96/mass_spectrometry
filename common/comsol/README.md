...
入口通过公共主机调度器依次申请 `prepare`、`solver`、`postprocess` 预算；未登记峰值的阶段按公共
策略保守独占，不根据瞬时低 CPU 利用率推断空闲。`-ResourceBudgets` 可传入冻结资源计划中的同名阶段
预算；预算仅控制 CPU、内存、I/O 与独占资源，不修改模型、求解器或物理参数。`-RunId` 只绑定运行身份。
公共策略是默认预算的唯一来源，本入口不维护另一套容量常量。
所有求解阶段强制追加同一个 `comsol-server-session` 具名独占资源，即使自定义预算未声明它也不能绕过。
服务器启动前的 PID 清单只在取得此锁后读取，锁一直保留到本次服务器清理完成；因此不会把排队期间
其他受管会话创建的服务器误判为本次新增进程。当前允许已知小型检查与 COMSOL 并行，不开放多个
受管 COMSOL 服务器会话并行。
