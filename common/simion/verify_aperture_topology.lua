...
local cell_mm = required_number('SIMION_APERTURE_CELL_MM')
...
assert(cell_mm > 0 and width_mm >= cell_mm and height_mm >= cell_mm,
...
local epsilon = 1e-9
local function lower_index(value, origin)
  return math.ceil((value - origin) / cell_mm - epsilon)
end
local function upper_index(value, origin)
  return math.floor((value - origin) / cell_mm + epsilon)
end

local ix_min = lower_index(flange_x_min, origin_x)
local ix_max = upper_index(flange_x_max, origin_x)
local iy_min = lower_index(center_y - width_mm / 2, origin_y)
local iy_max = upper_index(center_y + width_mm / 2, origin_y)
local iz_min = lower_index(center_z - height_mm / 2, origin_z)
local iz_max = upper_index(center_z + height_mm / 2, origin_z)
