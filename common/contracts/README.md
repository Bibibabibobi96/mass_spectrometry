...
它绝不删除未列出的兄弟目录；所有记录子范围完成后，才可移除已为空的`scratch/`根。台账保留外部范围记录并只扣除已处置字节。
将这类已批准证据放入`common/capacity_calibration/scratch_dispositions/`后，日常maintenance会复用同一入口完成处置并删除已消费的授权文件；没有批准证据的范围只在维护报告中保留，不会被推断删除。
