...
def _find_coupled_root(
    ...
    *,
    ...
    for iteration in range(1, bisection_iterations + 1):
        ...
        if abs(f_mid) <= 1.0e-16 or abs(hi - lo) <= 1.0e-12 * nominal_energy_v:
...
def source_position_samples(
    ...
    if sample_count < 1:
        raise PhysicsContractError("sample_count must be >= 1")
...
def _sample_summary(rows: Sequence[Mapping[str, float]]) -> dict[str, float | bool]:
...
def derive(contract: Mapping[str, Any]) -> tuple[dict[str, Any], list[dict[str, float]]]:
