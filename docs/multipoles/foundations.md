...
> **重要**
>
> 所有公式都依赖电压参考方式。本文中的 $V$ 是一个相位组相对共同轴偏置的 RF 零到峰值；共同偏置为零时即相对地。若硬件或论文给出相邻电极间差分值、杆对间值或峰峰值，必须先转换再代入公式。

按问题定位：[符号与单位](#2-坐标质量与几何约定) · [电压](#3-电压与波形约定) · [伪势](#6-rf-伪势与慢运动近似) · [模型层级](#13-模型保真度层级) · [来源](#16-参考资料)。
...
对真实圆杆结构，机器合同需要表达以下几何关系。下面是字段与数值示例，不是项目基线或现有 Schema：
...
相邻电极交替施加 $V_0+W(t)$ 和 $V_0-W(t)$，其中$V_0$是共同轴偏置，不进入理想二维横向场的
Mathieu参数。$V_0=0$时退化为相对地的$+W(t)$和$-W(t)$。本文默认：

| 量 | 定义 | 与差分电压的关系 |
|---|---|---|
| $V_0$ | 两个相位组共同的轴偏置 | 不改变理想横向差模场；有限端部中仍须显式建模 |
| $U$ | 一个相位组相对$V_0$的 DC 幅值 | 两相位组之间 DC 差值为 $2U$ |
| $V$ | 一个相位组相对$V_0$的 RF 零到峰值 | 两相位组之间 RF 零到峰值为 $2V$，差分峰峰值为 $4V$ |
| $V_{pp,\mathrm{diff}}$ | 两相位组之间差分峰峰值 | $V=V_{pp,\mathrm{diff}}/4$ |
| $\phi$ | 电压波形相位 | 必须明确参考时刻和参考电极组 |

> **警告**
>
> 不要只保存字段名 `rf_voltage_V`。机器合同至少要保存 `reference`、`amplitude_type`、`polarity_groups`、`waveform`和单独的`common_mode_offset_V`，否则存在 2 倍或 4 倍质量标尺错误。
...
每个多极杆 mode 应绑定模型、约定与假设。下面是表达示例；实际字段服从项目机器合同：
...
```json
{
  ...
  "conventions": {
    ...
    "voltage_id": "multipole.pair_about_common_mode.zero_to_peak.v1",
...
表中测试 ID 与数值容差是参考测试设计，不是自动授予资格的现有门禁；实际采用时按量纲与
数值精度建立项目合同，不得省略电压约定。
