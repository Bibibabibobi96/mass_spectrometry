...
按问题定位：[输入](#2-气体状态与必需输入) · [模型等级](#6-碰撞模型等级) · [验证](#14-参考验证矩阵) · [数据引入](#17-文献与数据引入) · [来源](#18-参考资料)。
...
C3 模型中，每个通道应表达下列信息。下面的物种、路径和阈值是示例，不能作为可执行数据或项目基线：
...
项目阈值应写入机器分析合同；理论文件只定义测试语义，表中 ID 不表示现有自动测试全部实现。

## 15. 机器合同示例

下列数据集 ID、路径和数值仅展示模型表达；使用前必须映射到项目实际 Schema 和已核验数据。
...
搜索得到的碰撞截面、迁移率或反应模型进入项目正式计算前，应完成：
