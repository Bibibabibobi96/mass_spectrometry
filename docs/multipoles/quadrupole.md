...
按问题定位：[Mathieu 推导](#3-从牛顿方程到-mathieu-方程) · [稳定区](#5-第一稳定区) · [质量尺度](#7-质量尺度) · [参考测试](#14-理论和数值参考测试) · [来源](#17-参考资料)。
...
$x$ 电极组施加 $V_0+W(t)$，$y$ 电极组施加 $V_0-W(t)$。$U$ 和 $V$ 都是一个相位组相对共同轴偏置
$V_0$的幅值，$V$为零到峰值；$V_0=0$时即原来的相对地约定。$V_0$不进入理想二维横向Mathieu参数，
但有限入口、出口和相邻静电电极中的轴向效应仍须在三维模型中验证。
...
> **重要**
>
> 上式中的 $V$ 是一个相位组相对共同轴偏置的 RF 零到峰值。若控制器给出两相位组之间的差分峰峰值，必须先除以 4。

## 8. 理想通带和稳定区分辨率

本节用 $k=2U/V$ 表示工作线斜率，避免与电荷极性符号 $s$ 混淆。工作线与两条边界的交点满足：

$$
kq_{\mathrm{in}}=-a_0(q_{\mathrm{in}}),
\qquad
kq_{\mathrm{out}}=b_1(q_{\mathrm{out}}).
$$
...
位置、尺寸、RF/DC 与结构误差的参数化定义见[公共误差族](foundations.md#12-公共误差族)。
四极质量过滤应特别检查峰位、峰宽、卫星峰、尾峰及损失方位，避免重复维护误差分类表。
...
表中尚未接入的测试 ID 是验证设计，不代表自动门禁已存在。FWHM、网格和统计方法以
[验证方法](../VALIDATION_METHODS.md)为准，具体阈值以项目分析合同为准；项目门禁明确要求跨求解器时，再将其作为强制条件。

当前最小L0参考实现位于RF项目[`workflows/mass_filter_reference/theory.py`](../../projects/rf_quadrupole_ion_optics/workflows/mass_filter_reference/theory.py)，覆盖本节尖点、RF-only截止、扫描线和质量尺度测试向量。它暂不提升到`common/`，也不覆盖单周期矩阵、有限长度轨迹、峰形或高阶场谐波。
...
现有 RF 四极杆项目入口为 [`projects/rf_quadrupole_ion_optics/README.md`](../../projects/rf_quadrupole_ion_optics/README.md)。该项目的几何、粒子源、mode 和接口仍由其 `config/` 单向派生，本文件不得成为第二参数源。
