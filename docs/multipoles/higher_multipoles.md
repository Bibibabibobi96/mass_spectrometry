...
按问题定位：[一般模型](#1-一般-2n-极模型) · [阶数比较](#4-四极六极与八极杆比较) · [接受度](#5-伪势深度可用半径与接受度) · [参考测试](#11-参考测试) · [来源](#13-参考资料)。

## 1. 一般 $2n$ 极模型

采用一个相位组相对共同轴偏置的 RF 零到峰值 $V$；以下省略不改变二维横向场的常数偏置。
理想 RF-only 势为：
...
项目、结构变体与 mode 的划分见[仓库架构](../REPOSITORY_ARCHITECTURE.md)，本节不另立目录规则。
...
下表定义理论参考测试语义；ID 和容差示例不代表已注册或已通过的项目门禁。
...
现有六极与八极杆入口由[理论索引](index.md)导航；新的碰撞设计线按仓库项目边界判定，
不从示例名字推定已存在或必须新建的项目。
