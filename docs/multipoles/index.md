...
本目录当前整体标记为 `provisional`：正文和主要数值已经过人工审查与独立复算。四极杆L0中的Mathieu稳定区、质量尺度和电压合同已有项目内求解器无关参考实现及自动测试；六极杆和八极杆也已共同验证一般$2n$极理想场、有限长度L1传输、二维圆杆COMSOL场谐波筛选及无端部L2传输。网格收敛、有限三维边缘场和求解器交叉验证尚未达到同等覆盖，因此知识包整体仍不提升为`reference`。
...
根仓库规则高于本目录。本目录不建立多级杆“总项目”。`common/multipole`仅保存已经由六极杆、八极杆两个平级项目共同调用并通过门禁的L1和二维圆杆L2参考实现；项目参数、机械几何和正式求解器资产仍由各项目维护。
