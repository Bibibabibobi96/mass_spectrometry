...
本目录当前整体标记为 `provisional`：正文和主要数值已经过人工审查与独立复算，但尚未建立仓库内的求解器无关参考实现和自动理论测试。后续只有同时具备固定来源、统一约定、参考实现和通过的测试，相关章节才能提升为 `reference`。
...
- $V$ 表示一个相位组相对地的 RF 零到峰值，差分峰峰值必须另存字段；
