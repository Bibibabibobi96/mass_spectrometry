...
本目录当前整体标记为 `provisional`：正文和主要数值已经过人工审查与独立复算。四极杆L0中的Mathieu稳定区、质量尺度和电压合同已有项目内求解器无关参考实现及自动测试，但一般$2n$极、单周期矩阵和高阶多极杆尚未形成同等覆盖，因此知识包整体仍不提升为`reference`。按Roadmap，首个实现保留在RF项目内；只有第二个多极杆项目实际复用验证后才评估上移`common/multipole/`。
...
根仓库规则高于本目录。本目录不建立多级杆“总项目”，也不预先建立尚未被第二个项目复用的 `common/multipole` 代码层。
...
当前 [RF四极杆项目](../../projects/rf_quadrupole_collision_cooling/README.md)可使用本知识包支持 RF-only、RF+DC 质量过滤和后续碰撞冷却。该项目当前是否具备 Candidate 或 Formal 资格，只查项目 `docs/PROJECT.md` 和机器门禁。
