...
本目录当前整体标记为 `provisional`。既有正文记录了人工审查与复算，但未逐条绑定可重跑证据；
自动参考向量、数值适用域和跨求解器验证覆盖尚未完整，因此不提升为
`reference`。具体实现的调用范围和验证状态不在理论入口重复维护。
...
知识归属按[仓库架构](../REPOSITORY_ARCHITECTURE.md)执行。本目录不建立多级杆“总项目”。四、六、八极杆的共享实现边界及当前调用方以
[`common/multipole/README.md`](../../common/multipole/README.md)为入口；项目参数、机械几何、当前验证状态和正式求解器资产仍由各项目维护。
...
文件按物理模型边界拆分，而不是按每个公式拆分。完整的 L0–L5 定义只在
[共同理论](foundations.md#13-模型保真度层级)维护，本入口不复制第二份定义。
...
坐标、质量、几何和 RF/DC 电压仅在[共同理论](foundations.md)定义；引用任一公式前先核对这些约定。
理论正文中的配置片段和测试 ID 是模型表达示例，不意味着存在同名 Schema、数据集或可执行门禁。
使用时查公共实现或项目机器合同，不从本文复制出第二套参数。

文末书目提供原始来源入口；尚未逐公式补齐章节/页码的引用保持待核验，不把书目存在当作逐项
推导验证。此次文档重组不提高理论成熟度，也不重新认证外部文献或图示数值。
...
- [仓库权威与知识路由](../REPOSITORY_ARCHITECTURE.md)
