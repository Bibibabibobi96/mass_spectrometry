-- Initialize and boundary-couple the accelerator-main fast-adjust PA family.
--
-- This is intentionally a role-specific counterpart to
-- build_accelerator_overlay_basis.lua.  It writes exactly the same outer
-- boundary values, but partitions the six faces into disjoint loops: no edge
-- or corner is revisited and no per-node string key is allocated.  Keeping it
-- separate avoids invalidating the already reusable upstream-bridge cache.
-- Usage is identical to build_accelerator_overlay_basis.lua.
...
for basis=0,maximum_electrode do
  ...
  -- y faces omit x edges, which were already written above.
  for iz=0,fine.nz-1 do for ix=1,fine.nx-2 do
    copy(ix,0,iz); copy(ix,fine.ny-1,iz)
  end end
  -- z faces omit both x and y edges, completing a disjoint surface cover.
  ...
  assert(count==expected, 'disjoint accelerator-main boundary traversal is incomplete')
  fine:save()
  total_boundary_points=total_boundary_points+count
  print(string.format(
    'ACCELERATOR_MAIN_BASIS: basis=%d boundary_points=%d coarse=%s fine=%s',
