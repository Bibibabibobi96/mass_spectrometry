% Solve three superposable field bases on the shared multipole connector geometry.
...
try
    ...
    expressions = {'-d(Vaxial,x)','-d(Vaxial,y)','-d(Vaxial,z)','Vaxial', ...
    ...
    samples = table(probeNames(:), coordinates(:,1), coordinates(:,2), coordinates(:,3), ...
        ...
        'axial_dc_Ex_V_per_m','axial_dc_Ey_V_per_m','axial_dc_Ez_V_per_m','axial_dc_potential_V', ...
    ...
    metrics = struct( ...
        ...
        'field_bases_solved', {{'axial_dc','rf_unit_100_V','oatof_pulse'}}, ...
...
electricForce.set('E', { ...
    sprintf('(-d(Vaxial,x))+%.17g*(-d(Vrf,x))*sin(2*pi*%.17g[Hz]*t+%.17g)', rfScale, frequency, phase), ...
    sprintf('(-d(Vaxial,y))+%.17g*(-d(Vrf,y))*sin(2*pi*%.17g[Hz]*t+%.17g)', rfScale, frequency, phase), ...
    sprintf('(-d(Vaxial,z))+%.17g*(-d(Vrf,z))*sin(2*pi*%.17g[Hz]*t+%.17g)', rfScale, frequency, phase)});
