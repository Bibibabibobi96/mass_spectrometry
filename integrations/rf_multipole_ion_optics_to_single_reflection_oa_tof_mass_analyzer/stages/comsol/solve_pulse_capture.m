% Track the multipole-exit population through PrePulse geometry under one finite pulse.
...
assert(all(cellfun(@isfile, requiredFiles)) && isfolder(oaComsolDir) && ...
    isfolder(multipoleComsolDir), ...
    'PulseCapture frozen inputs or solver-neutral COMSOL source are incomplete.');
...
try
    ...
    [model, comp, context, geometryInfo, meshElementCounts] = ...
        ...
        pre_pulse, resolvedConnection, sharedJoint, rf, oa, oaComsolDir, ...
...
events=cell2table(eventRows,'VariableNames',{'particle_id','event','status','terminal_reason', ...
    ...
    'x_mm','y_mm','z_mm','vx_m_s','vy_m_s','vz_m_s','rf_phase_rad', ...
