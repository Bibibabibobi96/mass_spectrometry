...
class SingleFlightApertureComparisonTests(unittest.TestCase):
    ...
    def test_pre_pulse_source_only_comparison_uses_full_mother_denominator(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            cases = {}
            for name, z_values in {"square_0p9": [0., 1., 2.], "round_2p5": [0., 2.5, 5.]}.items():
                run = root / name; (run / "inputs").mkdir(parents=True); (run / "results").mkdir()
                (run / "run_manifest.json").write_text(json.dumps({"status": "success", "run_id": name}), encoding="utf-8")
                (run / "summary.json").write_text(json.dumps({"status": "success", "role": "rf_oatof_simion_single_flight_summary"}), encoding="utf-8")
        ...
        square = result["cases"]["square_0p9"]
        self.assertEqual(square["mother_cohort_count"], 4)
        self.assertEqual(square["accelerator_entry_count"], 3)
        self.assertEqual(square["transmission_fraction_of_mother"], .75)
    ...
    def test_pre_pulse_source_only_comparison_rejects_empty_or_nonfinite_final_state(self) -> None:
        for bad_state in (
            pd.DataFrame(columns=["particle_id", "sample_index", "z_mm", "vz_mm_per_us"]),
            pd.DataFrame({
                "particle_id": [1, 2], "sample_index": [1, 1],
            ...
            with self.subTest(rows=len(bad_state)), tempfile.TemporaryDirectory() as temporary:
                ...
                for name, state in {
                    "valid": pd.DataFrame({
                        "particle_id": [1, 2], "sample_index": [1, 1],
                        "z_mm": [0.0, 1.0], "vz_mm_per_us": [1.0, 2.0],
                    }),
                    "bad": bad_state,
                }.items():
                    run = root / name
                    ...
                    state.to_csv(run / "results" / "pre_pulse_time_series_states.csv", index=False)
                    (run / "results" / "pre_pulse_time_series_screening_receipt.json").write_text(
                        json.dumps({"terminal_census": {"window_complete": {"count": 2}}}),
                        encoding="utf-8",
                    )
                    cases[name] = run
                with self.assertRaisesRegex(Exception, "needs two finite final pre-pulse states"):
