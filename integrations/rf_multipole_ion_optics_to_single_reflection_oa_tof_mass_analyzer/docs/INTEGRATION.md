...
半径100 mm、2 mm薄环、8/15环的新oaTOF候选接入受治理八极杆稳态源后，N=1000单次Fly得到
`1000→1000→1000→996→995`、单峰`R=23834.40`。它与此前完整稳态束上界`R=23901.04`基本相同，
并未复现同候选理想源诊断的`R=107739.80`；相对该理想源数值下降77.88%（4.52倍）。两者反射器
轴向网格分别为0.25 mm和0.10 mm，因此77.88%只作跨工况诊断幅度，不作严格同网格归因；但结果已
确认缩小反射器和薄环带来的理想源收益没有自动解决真实稳态束的`z-vz`时间聚焦失配。执行入口为
[`octupole_oatof_r100_t2_rings8_15_n1000_campaign.json`](../config/diagnostics/octupole_oatof_r100_t2_rings8_15_n1000_campaign.json)。
...
3. 保持d1=3.0 mm基准，以冻结真实z-vz相空间筛选repeller/grid1场比，再用完整稳态束确认；理论
   编译器必须自动重算聚焦面和反射器，不直接指定派生量。通过标准为N=1000、传输≥95%、单峰且
