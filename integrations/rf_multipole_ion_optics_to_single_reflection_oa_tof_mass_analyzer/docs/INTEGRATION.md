...
当前campaign的八极杆分段加速源与四极杆无加速5 eV源，除既有COMSOL接口运输外，又使用同一
SIMION 2020真实矩形侧端口、同一脉冲时钟和同一接口加速器PA完成了独立运输。正式计时run分别为
`20260803_235000__sim__simion__oct-oatof-interface-timed__n41`和
`20260803_235100__sim__simion__quad-oatof-interface-timed__n94`；两者使用的接口PA SHA-256均为
`99580E59D2E0546EB82CAC25F368398D9B2D3280805668C219C27B79B4CF3CFC`。表中差值均为
SIMION减COMSOL，并在各自的局部加速器出口平面统计：

|源工况|入口 / SIMION出口 / COMSOL出口 / 配对|质心x差 / mm|质心y差 / mm|质心RMS半径差 / mm|平均x角差 / °|角度RMS差 / °|平均能量差 / eV|能量标准差之差 / eV|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|八极杆分段加速|`41 / 34 / 33 / 33`|-1.0742|+0.0201|-0.5116|-2.7983|-2.0115|+223.5455|+9.7874|
|四极杆无加速5 eV|`94 / 34 / 35 / 34`|-0.7116|+0.0450|-0.7450|-2.3511|-2.7744|+212.2049|+16.1710|

这些差值明显超过当前多极杆家族工程推进参考量0.2 mm、1°和0.2 eV，因而当前SIMION与COMSOL
接口运输**没有闭合**；不能把任一求解器称为更准确，也不能据此授予收敛或资格。两种源都呈现相同
方向的约0.7--1.1 mm横向质心偏移、约2.4--2.8°平均x角差和约212--224 eV平均能量差，优先应核对
两端的电极边界、理想栅网处理、事件平面与脉冲电位时序，而不是先扩大多极杆参数扫描。

两条run都发布`logs/stage_timings.json`。共享PA复用时，从运行包建立到结果完成分别为41.617 s和
40.797 s；其中正式oaTOF资产校验、复制与程序组装分别占33.287 s和32.734 s，SIMION飞行分别占
5.002 s和4.800 s，接口PA复用准备仅约0.02 s。首次接口PA构建/refine另有独立资源日志，约204 s，
不得与复用run混为一项。当前常规重复运行的主要耗时是1.3 GB级正式资产本地化，不是粒子飞行。
所有结果保持`INCONCLUSIVE_DIAGNOSTIC_ONLY`。
