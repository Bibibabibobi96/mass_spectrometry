...
本页冻结2026-09-11文档整治前的工作区叙述，源为`docs/INTEGRATION.md`，原始字节SHA-256：
`f58548daaa02c8193f50262894cf0eeaf87fb0784e58ba2ca506f461632d3a70`。包含任务开始前已有的未提交RF作用域与IOB优先级修正记录；
归档仅保存来源，不代表其实现已经提交或所有验证闭合。为位置迁移只调整相对链接和原H1层级。

旧文中“当前”“下一步”“已闭合”均按原段落时点解释。尤其RF对象作用域与pre-pulse IOB优先级缺陷
已经使其对应的9月11日对照失去真实RF物理资格；早期离散结局一致不能覆盖这些后发现缺陷。
本次未重算粒子、未验证全部外部manifest、未修改预注册阈值或原始run。
当前状态只查[INTEGRATION](../INTEGRATION.md)，操作只查[RUNNING](../RUNNING.md)。
...
- 在启动带电场的 post-pulse 或连续全流程孔径扫描前，
  ...
  main family 的缓存身份与入口 local family；运行时按相应阶段 materialize 约 300 mm 的 main payload，且 local 的
  Dirichlet 来源必须绑定该 main cache。该检查本身不读取或哈希 PA 二进制，也不启动 SIMION。
...
- 20 环、300 mm 的孔径扫描固定主 PA 与其粗 Dirichlet 边界为同一 `1.0 mm × 1.0 mm` 参考孔；所有
  ...
  入口 local PA 覆盖主 PA 的参考孔局部，局部范围以外继续由已 refine 的约 300 mm 主 PA 提供与连续全程
  相同的真实 RF/DC 边缘场和碰撞几何。因此 pre-pulse 必须 materialize 并复用同一主 PA cache generation
  及由它建成的 local family，不允许用零场第一区载体或旧 0.25 mm pre-pulse 场合同替代。
- 完整母群的八臂连续全流程固定使用
...
共同说明运行副本并非原缓存字节。当前 continuous pre-pulse materialize 真实主 PA 与入口 local PA，
因此对这两个 PA+ 运行副本执行同一边界标记恢复；只有独立的 terminal-handoff collision-only raw-PA
路径不执行此操作。真实场对照已确认出口后
...
入口 local family 首次构建时同样不得把 immutable main generation 通过 junction、symlink 或 hard link 暴露给
SIMION；运行器使用公共直接清单物化原语，把 manifest 中的完整 8-mode family 复制到短 execution run 的可写
临时目录；复制流逐文件核对完整manifest，Dirichlet basis 只读取隔离副本，使用后删除副本。该步骤只复制，
不 refine 主 family，也不因供应商未打开只读源而重复全量哈希源 generation。
...
分区为未入选pulse、入选后检测、入选后损失；未入选者的脉冲关闭自然未来终态仅作诊断，不充当pulse-on损失。
...
本轮方形 h100 已按用户授权保留旧 generation，并把新的 8-mode 主 family 以 cache key
`0d29ca6ca921baedc6bbea626bf2cba2dbf1d5dd0919a445267ae1ec3e2fea12`、generation
`a7ba75aa916369417781883454d35286e8f19d8720461f3a5cc5f0f33b1c9fa4` 发布；只执行了一次
SIMION 官方默认 refine，完整 16 文件 inventory 独立复核 PASS。后续同身份运行只允许 materialize，
不得再次 refine。当前方形 h100 detector-blind pre-pulse `20260911_131548` 在 71.0 us 从同一冻结
5000 粒子母群选中 59 粒子（1.18%）；对应 restart/post-pulse `20260911_133007` 检测 52/59，折合
母群 52/5000（1.04%）。同钟连续 full-flight `20260911_133746` 已完成 5000/5000 自然终态，检测同为
52/5000，且检测粒子 ID 集、59 个配对粒子的最终检测/损失物理类别均与 restart 完全一致。
两条链的直接 TOF FWHM 分别为 144.0505297 ns 与 144.0007805 ns，mass resolution 分别为
93.17092678 与 93.20325506。随后对生成 Program 的作用域审计发现，这三次运行的 RF 对象被错误地
局部声明在 `segment.initialize_run()` 内，后续 `segment.fast_adjust()` 与 continuous 原生 RF 时间格回调
实际看到 `nil`；脉冲前杆电压因此没有随 RF 相位更新。上述 run 只保留为失效实现下的链路与对照诊断，
不得作为真实 RF 传输、脉冲效率、分辨率或孔径比较证据。修复不改变 PA 几何、电压 basis 或 refine 身份，
所以已发布 PA family 继续只读复用；方形 h100 必须从 pre-pulse 开始重跑。
修复 RF 作用域后完成的 pre-pulse `20260911_143308`、restart/post-pulse `20260911_161731` 与
continuous `20260911_164630` 又暴露出独立的 IOB 覆盖优先级错误：SIMION 在重叠位置采用槽号较高的
PA；七槽 continuous 的顺序为 `coarse < accelerator_main < upstream_bridge < local`，而旧四槽
pre-pulse 错排成 `coarse < upstream_bridge < accelerator_main < local`。因此 restart producer 选择的
397 个粒子中，continuous 同钟状态集合只覆盖 71 个，严格对照以
`continuous_pulse_trace_does_not_cover_restart_producer_set` FAIL；这三次运行只保留为实现缺陷诊断，
其中 397 粒子、post-pulse 1 次检测和 continuous 0 次检测均不得用于孔径、灵敏度或分辨率结论。
四槽权威顺序现已修正为 `1=coarse`、`2=accelerator_main`、`3=upstream_bridge`、`4=local`，生成
Program 的角色、PA+ 调压索引和分析角色图均由该合同派生。修复只改变重叠 PA 的选择优先级，不改变
任何 PA 几何、basis 或 refine identity；方形 h100 仍从 pre-pulse 重跑，并继续只读命中上述 8-mode
family，禁止重新 refine。
...
不得把初始化容差用作轨迹收敛标准；双方零检测也不构成到达时间等价证据。本轮真实对照的离散结局已如上
闭合，但严格脉冲状态对照仍为 FAIL：连续链相对独立 pre-pulse 冻结状态的最大位置、速度、能量差分别为
0.01511575663 mm、0.7654803142 m/s、0.002256526005 eV；restart 注入相对冻结状态三项均为零。
共有检测粒子的最大到达时差为 0.0005597848 us，最大 x/y 落点差为 0.0582595499/0.02147064474 mm。
在建立独立轨迹误差预算或消除两套 pre-pulse 数值路径差异前，只能声明离散结局与总体峰指标一致，不能声明
逐状态或逐轨迹数值等价。该严格 FAIL 不靠放宽 restart 序列化容差改写。
...
圆形四孔径仍缺当前真场 family、自然轨迹、post-pulse 与连续全流程证据；按 h100、h150、h200、h250
顺序推进，同一圆形截面共享主 family，各孔径只新增入口 local family。
...
`3=主加速器细 PA`、`4=入口局部替换 PA`。slot 4 以最高优先级使用与七实例 continuous full-flight
完全相同的 active bounds、PA+ family，并在脉冲前投影入口电极 33/34 的静态 8 V；局部范围之外由
slot 3 的同一已 refine 主 PA 提供真实 RF/DC 边缘场，不能回退为零场碰撞载体。该最小可达 IOB 会
materialize 粗前端、上游细域、主加速器和入口 local 四套实际 PA，但不 materialize 反射器、下游飞行管
或探测器 PA，也不保留空实例。粗前端、上游细域、主 PA generation 与入口 local family 均按当前
`0.5 × 0.5 × 0.1 mm` main/local profile 的内容身份复用；旧零场第一区载体和 0.25 mm pre-pulse
合同均失败关闭。历史三实例 compact 结果只保留为缺失入口局部静态边缘场的缺陷证据，不能与连续全程作严格配对。
...
SIMION 2020 没有可用的公开 Lua 实例删除接口，因此运行器冻结一个仅含四个实例的版本受控二进制种子；加载时
先用同名临时 PA 满足种子，再立即替换为上述四个真实角色。该种子只表达 Workbench 实例数，不表达电场、几何
...
```powershell
$Repo = 'C:\Users\Liao\mass_spectrometry\simulation_repo'
...
& pwsh -NoProfile -File $Execute -Campaign $PreCampaign -ExperimentId $PreId -Exploration -SolverAuthorized
```
...
& pwsh -NoProfile -File $Execute -Campaign $DerivedCampaign -ExperimentId $PostId -Exploration -SolverAuthorized
```
...
$ProducerMap = Join-Path $env:TEMP 'rf_oatof_pre_producer_map.json'
$PostMap = Join-Path $env:TEMP 'rf_oatof_post_producer_map.json'
@{$PreId = $PreOuter} | ConvertTo-Json | Set-Content -LiteralPath $ProducerMap -Encoding utf8NoBOM
@{$PreId = $PostOuter} | ConvertTo-Json | Set-Content -LiteralPath $PostMap -Encoding utf8NoBOM
$ContinuousCampaign = Join-Path $Integration 'config\explorations\square_h100_handoff_continuous_control_8mode_current_n5000.json'
& $Py -m integrations.rf_multipole_ion_optics_to_single_reflection_oa_tof_mass_analyzer.workflows.family_source_closure.author_full_flight_campaign_from_pre_pulse `
  ...
  --campaign-id square_h100_handoff_continuous_control_8mode_current_n5000 `
  --workspace $Workspace
$FullId = $PreId.Replace('_pre_pulse_', '_full_flight_')
& pwsh -NoProfile -File $Execute -Campaign $ContinuousCampaign -ExperimentId $FullId -Exploration -SolverAuthorized
```
...
资源身份还包含实际 IOB 工作负载拓扑；四实例 pre-pulse、五实例 post-pulse/轴场导出和七实例全程飞行
...
当前 20 环、300 mm 三区候选仍是 `CANDIDATE_ONLY`。活动完整母群配置的机器权威是
`frontend_xy025_z010_coarse100_full_bore_main_local_xy050_z010` 与主域 `full_accelerator_v1`：上游细域为
0.25 × 0.25 × 0.1 mm，粗前端为 1 × 1 × 1 mm，主域和入口 local 均为 0.5 × 0.5 × 0.1 mm。
旧 `coarse_boundary_supported_full_axial_core_v1` 与 `directed_kinematic_corridor_v1` 只保留为既有诊断身份，
不是当前物理链的主 PA；每个新截面仍须以总轴场导出和 N=1 轨迹验证，方可把后续 N=5000 结果用于该截面。
