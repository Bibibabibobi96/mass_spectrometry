...
原因和适用范围应写入正确文档。缓存、锁/恢复文件、零字节文件、临时日志、已确认失败或中断运行
的产物、被成功重算替代的旧结果，以及明确可再生的中间CSV、图片、轨迹、场表和中间模型可由
Agent直接删除，无需逐次确认，并在阶段报告中记录。只有用途、归属、可再生性或正式状态不确定
时才在删除前列清单请求用户决定；人工编辑文件、唯一正式模型和不可可靠重算文件不得自动删除。
...
该门禁验证R2025b可执行文件、SolidWorks 2022 PIA/COM revision，以及所有项目README均声明此
基线；它不重写或重存已有的MPH、SLDPRT或SLDASM。
