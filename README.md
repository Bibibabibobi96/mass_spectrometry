...
归档按信息职责和生命周期决定，不按操作者、软件或文件出现顺序决定。所有项目统一使用以下路由：

|内容|唯一位置|生命周期|
|---|---|---|
|当前跨软件状态、正式/候选边界、开放任务|项目`docs/PROJECT.md`|持续更新，只留当前事实|
|单软件当前实现、限制、简要失败矩阵和绕行|项目`docs/COMSOL.md`、`SIMION.md`或`CAD.md`|持续更新，不写逐次尝试流水账|
|机器参数、分析定义、冻结指标|项目`config/`|版本化机器权威|
|可复现入口、测试和门禁|项目源码、`tests/`或`common/`|按脚本生命周期维护|
|每次运行的原始模型、日志、崩溃转储、CSV、图片和结构化摘要|Git外`artifacts/projects/<project>/`|由run manifest固化|
|已关闭、被取代或达到明确调查里程碑的完整过程|项目`docs/history/`|一次归档后只读|
|厂商/标准原文|`official_docs/`|原样保存并建立索引|
|至少两个项目验证过的稳定方法|根`docs/`或`common/`|跨项目权威|
...
`hs_err_pid*.log`、MATLAB crash dump等工具日志，移动到该次运行目录后再写manifest。仓库根目录和
源码树不得充当scratch。意外产生的原始文件若无法唯一归属，先移入项目`scratch/<software>/`并在
...
保留策略面向可复现性：`formal`只保留当前门禁通过资产，`candidates`只保留仍可能提升的候选，
`archive`保留被正式引用的旧资产，`runs`保留被文档引用或用于失败根因的运行，`scratch`不作为引用
来源。删除仍遵守`AGENTS.md`的用户确认规则；“已进入history”本身不授权删除原始证据。
...
git diff --check
git diff --stat
.\common\verify_repository_hygiene.ps1
```
