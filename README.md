...
各目录的状态、保留与清理条件由本节末尾统一定义。脚本必须通过项目路径解析器定位这里，禁止硬编码
...
来源，`results`只保存可由已归档输入复算的后处理结果。删除仍遵守`AGENTS.md`的用户确认规则；
...
文档/仓库卫生门禁、oa-TOF Static门禁和RF Static门禁；本机默认使用`.venv`，GitHub Workflow注入
干净运行器的Python路径。门禁逻辑只在该脚本及其调用的项目门禁中维护；
